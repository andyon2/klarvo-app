...
| 2026-06-28 | Task 6 (AC-6 `overlayScale`) implemented (7fac265): config field default 1.0, multiplied into render scale in both overlay threads (`scale = dpi_scale × overlayScale`, coupled model). Review CLEAN (direct, Opus). Machine-verified: Linux `cargo check` clean + golden-master roundtrip passes (field persists) + win-gnu scratch harness clean recompile 0 errors + signature/call-site consistent. At 1.0 = pixel-identical to today. Status `review` pending Andi's tuning-smoke (dial overlayScale to taste). |
