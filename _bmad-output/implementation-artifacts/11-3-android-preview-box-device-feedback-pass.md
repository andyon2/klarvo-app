...
| 2026-07-08 | **b+c dev round + code-review + fix + device-verify.** Dev (`b50e439`): removed rolling-window (`visibleLines`/pool/`ROLLING_*`, deleted `RollingWindowVisibleLinesTest`), restored a bounded `TranscriptScrollView` inside the fixed-height panel with a pure `isScrolledToBottom` + stick-to-bottom rule. Code-review (3 reviewers): Auditor clean; 2 Edge-verified false positives dismissed (cross-recording reset via panel recreation; applyAppearance show-time only); 3 confirmed fixes in 1 round (`6b0c7d4`): F1 re-check `stickToBottom` at scroll-exec time, F2 one-shot `OnLayoutChangeListener` so auto-scroll fires AFTER re-layout (guarantees newest line visible), F3 load-bearing inversion test. Full JVM suite green, clean compile, KlarvoTheme in sync. **Andi real-device verified 2026-07-08 (fresh APK via Tailscale adb, freshness-checked): box follows newest speech ✅, manual touch-scroll works ✅, scrolled-up user not yanked ✅ → „passt".** **GATE-3 close-out: both status fields → `done`.** Accepted low residual (Andi ok): blinking caret scrolls out of view with the content (logged `docs/backlog.md`); scroll stick-threshold (24dp) device-tunable. AC-6 remains split to Story 11-4 (bubble z-layering). |
