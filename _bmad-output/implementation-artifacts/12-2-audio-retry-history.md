...
| 2026-07-06 | **Andi real-machine gate PASSED on BOTH platforms** — surface residual cleared. Android: fresh APK on real Xiaomi, pending entry + re-process + discard all green. Windows: fresh signed build; outage state produced via invalid Groq key in config.json (401 → terminal → pending entry appeared), re-process (key restored) + discard both green. No residual left for 12-2. | Andi (device smoke) + Claude |
