...
- 2026-06-05: Cosmetic revision per Andi (see "Cosmetic Revision" section below) — box now grows UPWARD above the pill (pill stays anchored), auto-grows with text height instead of scrolling, capped to screen so it never runs off the top. `npm run build` PASS (0 errors). `src-tauri/src/commands/misc.rs` touched (panel region now reads dynamic `inner_size()`) — this Windows-only `cfg` path was already **Windows-compiled+verified on Andi's screenshot-build** (the dynamic-height box hugged the text; impossible if the `inner_size` region failed). A follow-up `PANEL_WIDTH` 220→320 tune is TS-only and **not yet rebuilt** on Windows.
...
## Cosmetic Revision (2026-06-05) — grow UPWARD + auto-height, NOT scroll

Andi's three cosmetic tweaks (given in this session), implemented. This **revises AC-2/AC-3 semantics**: the panel no longer expands *downward* into a *fixed-height scrollable* box — it now sits ABOVE the pill and grows with the text.

1. **Box expands ABOVE the pill, not below.** The pill stays its anchor; the preview text grows upward. Mechanism: the pill anchor `barY` is held fixed, and on every resize the window is repositioned to `(barX, barY − panelHeight)`, so the window's bottom edge (the pill) stays put while the top edge rises. The panel is now the FIRST flex child (top); wrapper uses `justifyContent: flex-end` so a transient undersized window clips the OLDEST text, never the pill control. Drag stores the pill anchor (`onMouseUp` adds `panelHeight` back to the window top when the panel is open).
2. **Auto-grow with text, no scroll.** Panel height is the MEASURED text height, not a fixed 160 px. A hidden probe div (`measureRef`) mirrors the panel's text box at the final `PANEL_WIDTH` and is measured in a `useLayoutEffect` → no window-width feedback loop. Window height = `PILL_HEIGHT + panelHeight`; the OS region tracks it because `set_bar_shape("panel")` now reads the window's actual `inner_size()` (was hardcoded 220×196). `PANEL_HEIGHT`/`PANEL_MAX_HEIGHT` constants removed.
3. **Screen limit / fallback.** Height is capped to `min(PANEL_ABS_MAX=320, barY − SCREEN_TOP_MARGIN=12)` so the upward growth never crosses the top of the screen. ONLY past the cap does the box switch to scroll (`panelScrolls`) + auto-scroll-to-newest + top-fade mask (the fade is now applied only while scrolling, so a short fully-visible preview keeps its first line crisp).

**Tunables** (adjust in the Windows smoke if needed): `PANEL_ABS_MAX` (320, absolute max box height), `PANEL_WIDTH` (**320** — bumped from 220 after the first smoke: 220 gave a too-narrow ~30-char measure that fragmented flowing speech into stubby 4-word lines and made the bar a chunky near-square block; 320 ≈ 50 chars/line keeps it a wide, flat strip that matches the horizontal pill), `SCREEN_TOP_MARGIN` (12).
