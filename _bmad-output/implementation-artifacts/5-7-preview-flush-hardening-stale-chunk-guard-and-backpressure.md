...
### Windows smoke — round 1 (2026-06-05, Andy) → BLOCKED, handed to a fresh debug session

- **Finding:** after a few pause-flushes the preview panel clipped the OLDEST (top) line (hard cut, no top-fade → `panelScrolls=false` → grow-upward mode, OS window under-applied the grown height). Screenshot confirmed: full text present, window ~1 line too short. Andy: "wieder zerschnitten" (the recurring 5-5-family clip, now on accumulation).
- **Root cause hypothesis (pre-existing, NOT caused by 5-7):** the 5-2/5-5 grow-upward async `setSize`/`setPosition` race on the separate bar window. 5-5's `geomTick` re-assert (rAF + ~120ms) fires only on the **panel-open** transition (`useEffect([isPanelOpen])`); each **subsequent height growth** (text accumulating over several pauses) is left to a single async `setSize` that can under-apply → window height lags `panelHeight` → top-clip. 5-7 never touched this geometry code (its only FloatingBar change is the listener guard, which cannot affect height).
- **⛔ Smoke-fix r1 ATTEMPTED → MADE IT WORSE → REVERTED.** Tried widening the `geomTick` dep `[isPanelOpen]` → `[isPanelOpen, panelHeight]` so the re-assert fires on every grow step. Andy rebuilt and reported it got **worse** (not better). **Reverted** via `git checkout -- src/FloatingBar.tsx` — `FloatingBar.tsx` is back at the `e71d1c0` baseline (original `[isPanelOpen]`). **Likely why it backfired (hypothesis for the next session):** firing the rAF+120ms re-assert on *every* `panelHeight` change storms `setSize`+`setPosition` IPC on the separate window during rapid accumulation → the window keeps re-jumping/re-sizing, plausibly amplifying the clip/flicker instead of converging. So "re-assert harder" is the WRONG direction — the next session should look at the measure↔resize↔setPosition coordination (atomicity of size+position for an upward-growing window; whether `panelScrolls`/cap should engage earlier; or sizing from the bottom-anchored geometry differently), not at re-asserting more often.
- **STATUS: 5-7 stays `review`.** The 5-7 hardening itself (Commit `e71d1c0`: stale-chunk guard + in-flight cap) is **verified-good and orthogonal** — 572 tests/0 fail, inversion RED-verified. The blocker is this **separate, pre-existing grow-upward accumulation top-clip**. Andy stopped the conductor here (2026-06-05); a **fresh session must debug the geometry** from the clean `e71d1c0` baseline.
- **For the next session:** the relevant code is `FloatingBar.tsx` lines ~373-483 (measure `useLayoutEffect`, `activePillWidth/Height`, the `geomTick` re-assert, the resize/`setSize`/`setBarShape`/`setPosition` effect) + the render at ~734-778 (`panelScrolls`, `height:panelHeight`, `overflow`, `justify:flex-end`). Auto-scroll-to-bottom already exists (lines ~341-346). The stale-bleed smoke scenario (5-7's own deliverable) was NOT yet observed — the clip blocked it. Consider splitting the geometry clip into its own story/defer so 5-7 can close on its merits once the stale-bleed path is confirmed.

### Geometry debug — rounds 2–4 (2026-06-05): THREE speculative fixes, all WRONG, ALL REVERTED

Honest record: three confident fixes built on incomplete understanding, each verified only by a Windows build, the third **worse than baseline**. Trust cost was real. All reverted to the `e71d1c0` baseline via `git checkout HEAD -- <code files>`.
- **r2 (region read-back):** hypothesised the OS region (`set_bar_shape` reading `bar.inner_size()`) was stale; passed explicit dims instead. **Disproved by the instrumented log:** `actualInner == target` at every resize — the OS window/region size is always correct. Not the bug.
- **r3 (measure real panel in useLayoutEffect):** measured `previewPanelRef.scrollHeight` instead of the probe. **Made it persistent:** the log showed `panelScrollH=131` for 160 chars — measured while the window was still at the ~200px pill width (the resize to 400 had not landed), so the panel wrapped ~2× too tall.
- **r4 (fixed wrapper width):** pinned the wrapper to `PANEL_WIDTH`px. Reported worse on the build; reverted. (Likely the fixed-width card overflowed the not-yet-resized narrow window horizontally; stacked on r3.)

### Geometry debug — methodical reset + browser harness (2026-06-05) → ROOT CAUSE PROVEN, Guard #1 applied

Switched method (Andy's call): stop building speculative fixes on Windows; make the bug **observable on WSL** instead. The bar frontend is pure CSS/DOM and the log proved the Tauri window size is always correct, so the defect reproduces in **Chromium on WSL** with zero Windows builds and no human in the loop. Harness: `/tmp/klarvo-bar-harness/` (Playwright + Chromium, replicates the wrapper + `#preview-panel` + measure-probe box model verbatim).
- **Static box model is FINE:** at a settled width the probe and the real panel measure **identically** (both `offsetHeight=148`, text width `370`). So the bug is **dynamic**, not a static box-model divergence.
- **Mechanism PROVEN by a width sweep** (same text, varying window width): the probe (fixed `PANEL_WIDTH-2`) is constant at **82**; the real panel `scrollHeight` is **132 @ 200px → 115 @ 260px → 82 @ ≥320px**. The panel only diverges when measured at a width narrower than the settled width. `132 @ 200px` ≈ r3's observed `131` → confirms r3 measured at pill width.
- **Root cause (ground-truth-backed):** the **WebView2 CSS viewport reflow LAGS the Tauri window resize** (it trails even past `inner_size()` reporting the new size). When panel geometry is measured/rendered during that lag, the panel wraps narrower → taller than the measured `panelHeight` → and the **unconditional auto-scroll-to-bottom** (`scrollTop = scrollHeight`) pushes the OLDEST (top) line up and out under `overflow:hidden`, which did not reliably heal. The auto-scroll, not the measurement, is the proximate cause.
- **Guard #1 applied + offline-validated:** gate the auto-scroll to `panelScrolls` (capped only). While uncapped the box already grows to fit, so scrolling is unnecessary; keeping `scrollTop=0` means the top is **never** clipped (offline validation: top-clip during the simulated 200→400 lag goes from `scrollTop=50` baseline → `scrollTop=0` fixed). Single change to the auto-scroll `useEffect` in `FloatingBar.tsx` — measurement, window geometry, region, and `set_bar_shape` all **untouched**. `tsc --noEmit` green.
- **STATUS: 5-7 stays `review`.** NEXT: ONE canonical Windows build (`scripts/sync-and-build.ps1`) + smoke — (1) multi-pause accumulation no longer top-clips at comfortable AND wide, (2) 5-7's own stale-bleed scenario (observable once the clip is gone). Residual risk: the small ~1-line probe-vs-real difference seen in WebView2 (not reproduced in Chromium, likely Inter font) could leave the *newest* line briefly below the fold; if so, Guard #2 (re-measure the real panel on viewport settle via ResizeObserver) is the separately-validated follow-up. Change is local on `v1-ship`, **uncommitted** (await smoke before commit).
