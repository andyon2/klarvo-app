...
- [x] [Smoke][RESOLVED — verified Windows dev build, Andi, 2026-06-06] Preview window never appears. The 1×1→400×600 size fix (991c401) was necessary but NOT sufficient; re-smoke still showed no box. The console-to-log bridge (37c366b) made the true cause observable and diagnosis switched from elimination-guessing to reading the log. **Two real root causes, both behind the never-firing event path:**
  1. **Capability scope** — `capabilities/default.json` listed windows `["main","bar"]`, omitting `"preview"`. Custom `#[tauri::command]`s need no permission (so `invoke`/`frontend_log` worked), but `core:event:allow-listen` is capability-gated → the preview window's `listen()` was silently denied → no events → `runShowSequence` never fired. Log signature: `subscribing…` logged, `listener REGISTERED` never did. **Fix:** add `"preview"` to the windows array.
