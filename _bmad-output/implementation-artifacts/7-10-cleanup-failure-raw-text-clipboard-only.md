...
### Review Follow-ups (AI) — AC8 fix round 2, the FINAL round of the AC8 cycle (2026-09-14)

Scope is fixed by Andi: the confirmed items **1, 2, 3, 5** of the `d778d5c` re-review plus directive **DN1** on
the re-review's one decision item. Item 4 (canon / MANIFEST / mock) was already carried by the conductor in
`b67e297`. Nothing from the deferred or dismissed lists is in scope.

- [x] [AI-Review][Decision→DN1] **The flip threshold is ONE card height, not the 600 px window cap.** Andi,
      2026-09-14, verbatim: *"Die Flip-Schwelle ist EINE Kartenhöhe (`MIN_CARD_H_LOGICAL`, die Höhe einer
      vierzeiligen Karte), nicht die Fenster-Obergrenze von 600 px. Die Karte und die Live-Preview kippen nur
      dann unter die Pill, wenn oben weniger als diese Kartenhöhe frei ist."* → option (a) of the re-review's
      decision item [`src-tauri/src/native_preview.rs::compute_preview_geometry`]
- [x] [AI-Review][High] **Item 1 — the `STATE_RECORDING` branch must actually take the card off the screen.**
      P1's hold guard turned the `set_message(None)` that used to hide it into a no-op, so `dismiss_message`
      clears the state while the dead card stays painted through the whole next run whenever live preview is off
      [`src-tauri/src/native_preview.rs::preview_wnd_proc`, `WM_PREVIEW_SET_STATE`'s `STATE_RECORDING` arm]
- [x] [AI-Review][Med] **Item 2 — "raw text is in History" is asserted while the History write is best-effort.**
      Thread the write's outcome through to the card and word it conditionally
      [`src-tauri/src/overlay_message.rs::DegradeCause::ClipboardWriteFailed` + `src-tauri/src/pipeline.rs`'s
      history block, `terminal_degrade_cause` and the `done_with_clipboard_only` emit]
- [x] [AI-Review][Low] **Item 3 — `ERROR_TOKEN_TEXT`'s provenance comment is false and half its table is dead.**
      `require_license!` yields neither mapped token; `CommandMode` comes from `pipeline.rs`, `OfflineMode` never
      becomes a card. Correct the comment, drop the `OfflineMode` row, fix the D3 resolution row in the record
      [`src-tauri/src/overlay_message.rs::ERROR_TOKEN_TEXT` + story Dev Agent Record, D3 row]
- [x] [AI-Review][Low] **Item 5 — two comments falsified by P1 in the commit that wrote it**
      [`src-tauri/src/native_preview.rs::preview_wnd_proc` `STATE_RECORDING` comment +
      `src-tauri/src/hotkey/mod.rs::tests`, the docstring above `spec_progress_events_carry_no_card`]
...
### Review Findings (bmad-code-review RE-REVIEW, 2026-09-14, range `6cedbb5..HEAD`, fix commit `d778d5c`)

**Narrow mandate, stated up front.** This round verifies ONLY that the 14 items of the `8b81864` review (D1–D3 + P1–P11) are
resolved and that the lines `d778d5c` touched regressed nothing. **No fresh full adversarial sweep was run**; anything
found outside that scope is recorded below as residual, not acted on. Three layers ran, none failed. `d778d5c` is the last
commit that touches code — `c8d9655` is docs + the Windows-build exit file, `bd0c406` is seat-costs.

**Verification verdict on the 14 items: 13 resolved, 1 resolved-but-over-reaching (D1), 1 resolved-with-a-new-regression (P1).**
P2, P3, P5, P6, P7, P8, P9, P10, P11 are clean — each proved by a named symbol. P4 and D3 are resolved in behaviour but ship
an inaccurate comment (see below). D2's card is correct in tone/header/wording per Andi's directive; its new *History*
sentence is the one promise that is not conditional on anything.
**1 decision-needed · 5 patch · 12 deferred · 14 dismissed as noise.**

- [x] [Review][Decision] (Andi 2026-09-14 → **DN1**, option (a), applied in fix round 2) **D1's flip triggers on the window cap, not on the card height — so the live preview moves below the pill for a large band of ordinary pill positions.** `native_preview::compute_preview_geometry` computes `below_pill = avail_above < h_wanted && avail_below > avail_above` where `h_wanted = (BASE_MAX_HEIGHT * k).round()` — `BASE_MAX_HEIGHT` is **600** logical px, the *window's maximum* height, not the height of the card that has to fit. Andi's directive is *"Wenn oben weniger Platz als die **Kartenhöhe** ist, erscheint die Karte 8 px UNTER der Pill"*; a four-line message card is roughly 110 px. Worked example, 1080p, scale 1, work area ≈ 1040 logical px, pill dragged to `y = 400`: `avail_above = 380`, `avail_below = 584` → `380 < 600` and `584 > 380` → **the whole window flips below the pill**, although 380 px above is more than three times what the card needs. Because `below_pill` also drives `render_frame`, the **live preview** — which has always grown upward above the pill — now renders *below* it for every pill position in roughly the upper half of the screen. The fix that closed a top-edge corner case changed the default placement of an existing surface. The record states the threshold openly ("*less than the card wants (`BASE_MAX_HEIGHT × k`)*") but does not flag it as a substitution for the directive's wording. Options: (a) flip on an approximate card height — a new `MIN_CARD_H_LOGICAL` sized to the four-line card — so the flip stays the top-edge case the directive names; (b) keep today's threshold and accept the preview sitting below the pill in the upper screen half; (c) two thresholds, card vs live preview — contradicts the directive's *"gleiches gilt für die Live-Preview in dieser Lage"*. Needs Andi's UX call, because it re-touches his own D1.

- [x] [Review][Patch] (item 1, fixed in fix round 2) P1's hold guard leaves a stale card on screen when a new recording starts inside the 4 s hold — `dismiss_message` clears state but never repaints and never hides, and the `set_message(None)` that used to do the hiding is exactly what P1 now swallows; with `live_preview_enabled: false` (AC8's headline case) nothing repaints until the run's terminal state, so the dead failure card covers the whole record+STT+LLM cycle. This re-opens the same round's deferral *"the `STATE_RECORDING` branch dismisses the card but neither repaints nor hides — deferred, unreachable via `emit_pipeline_state`, which posts its own `set_message(None)` first"*: P1 deleted that premise. Breaks AC8's "a new recording … dismisses it at once" [`src-tauri/src/native_preview.rs::preview_wnd_proc`, `WM_PREVIEW_SET_STATE`'s `STATE_RECORDING` arm + `native_preview::dismiss_message`]
- [x] [Review][Patch] (item 2, fixed in fix round 2) D2's card asserts "raw text is in History" unconditionally, but the History write is best-effort and its outcome is never consulted — `if let Ok(db) = state.history_db.lock()` skips the write silently on a poisoned mutex, and an `add_entry` error is swallowed into `log::warn!`. This is the same false-promise shape review round 1 (F6) removed when it deleted the clipboard promise from this very variant. The outcome *is* available in time: the write happens before the terminal event is emitted, so it can be threaded into the cause instead of asserted [`src-tauri/src/overlay_message.rs::DegradeCause::card`, `ClipboardWriteFailed` arm + `src-tauri/src/pipeline.rs::stop_and_process_pipeline`, the history block and the `done_with_clipboard_only` emit]
- [x] [Review][Patch] (item 3, fixed in fix round 2) D3's provenance comment is false and half the shipped table is unreachable from a card — the KDoc names "`lib.rs`'s `require_license!`" as a source, but `require_license!` only ever produces `WhisperMode` · `AppProfiles` · `Snippets` · `Sync` · `UnlimitedHistory` · `FillerAnalysis` · `VoiceNotes` · `AlternativeProviders`, i.e. **neither** of the two mapped tokens. `CommandMode` reaches a card from `pipeline::process_audio`'s command-mode gate; `OfflineMode` exists only as a Tauri command `Err(String)` in `commands/whisper.rs`, which never becomes an `OverlayMessage`. The test's "unknown token" example `feature_requires_license:Sync` is in fact a real token in the tree. The story's D3 resolution row repeats the same grep summary [`src-tauri/src/overlay_message.rs::ERROR_TOKEN_TEXT` doc comment + story Dev Agent Record, D3 resolution row]
- [x] [Review][Patch] (item 4 — carried by the **conductor** in `b67e297`, NOT by fix round 2; nothing here re-verifies it) Canon and MANIFEST were not updated for D1's below-pill placement or D2's `TEXT LOST` danger variant, although AC8 names them the source of truth — the 7.10 entry still reads "*die Preview-Karte (8 px über der Pill, Amber-Linie)*" with no below-pill case and no danger-card variant, and `mockup-7-10-message-card.html` has no `TEXT LOST` scene. Rounds 1 and 2 of this story patched canon + MANIFEST for exactly this class of change [`docs/design/overhaul/source/Klarvo Design System.html`, the 7.10 `<li>` + `docs/design/overhaul/source/MANIFEST.md` + `docs/design/overhaul/mockup-7-10-message-card.html`]
- [x] [Review][Patch] (item 5, fixed in fix round 2) Two comments were falsified by P1 in the same commit that wrote the guard — the `STATE_RECORDING` arm still says "*Its own `set_message(None)` arrives just before this, so this is the belt to that braces*" (P1 made that `set_message(None)` a no-op inside the hold), and the spec docstring above the progress-state test still says "*the overlay clears any card still on screen when the next run starts*" [`src-tauri/src/native_preview.rs::preview_wnd_proc` `STATE_RECORDING` comment + `src-tauri/src/hotkey/mod.rs::tests`, the docstring above `spec_progress_events_carry_no_card`]

- [x] [Review][Defer] The card can still render **German** — `lib::friendly_error` returns `" Groq Free-Tier-Limit erreicht — warte einen Moment und versuche es erneut."` for any rate-limit error, and `pipeline::generic_degrade_cause` puts that string straight into `DegradeCause::Generic`, which the card renders as its cause line. Canon pins "Alles englisch" [`src-tauri/src/lib.rs::friendly_error` → `src-tauri/src/pipeline.rs::generic_degrade_cause` → `src-tauri/src/overlay_message.rs::DegradeCause::card`] — deferred, **pre-existing since `8b81864`** and outside this round's mandate; the earlier review missed it. A real AC8 violation, worth its own item
- [x] [Review][Defer] `warn_all` puts an embedded `\n` into the scalar `warning` field, which also travels to the main-window status line where HTML collapses it to a space [`src-tauri/src/hotkey/mod.rs::PipelineEvent::warn_all`] — deferred, **inert today**: verified that a boot warning never reaches that surface (`useRecording.ts` returns early on `state === "warning"`, and `App.tsx` renders `warningMessage` only when `recordingState === "done"`)
- [x] [Review][Defer] D1 kept `phys_h.max(1)` and added no minimum-height floor, so the original silent-loss shape survives wherever *both* sides are tiny [`src-tauri/src/native_preview.rs::compute_preview_geometry`] — deferred, needs a work area barely taller than the pill; the record names it, and Andi chose option (a) not (b)
- [x] [Review][Defer] `work_bottom` is captured once at `preview_thread` start and never refreshed, but D1 makes it drive a *placement* decision for the first time — a taskbar move, resolution or DPI change can now pin the window on the wrong side of the pill, not merely mis-clamp its height [`src-tauri/src/native_preview.rs::preview_thread`] — deferred, needs `WM_DISPLAYCHANGE`/`WM_SETTINGCHANGE` handling; the record names the primary-monitor `SPI_GETWORKAREA` limitation but not the staleness
- [x] [Review][Defer] No hysteresis on the flip — `avail_below > avail_above` is a strict compare of two continuously-varying quantities, recomputed on every `WM_PREVIEW_SET_PILL_POS`, so dragging the pill through the crossover snaps the window between above and below [`src-tauri/src/native_preview.rs::compute_preview_geometry`] — deferred, cosmetic during a drag
- [x] [Review][Defer] The already-deferred "overflow lines drawn outside the card" bug changes character under `below_pill`: bottom-aligned, overflow ran off the window edge and was clipped; top-aligned in a window sized to `avail_below`, it now lands inside the window but outside the card, as floating text on the transparent background [`src-tauri/src/native_preview.rs::render_message_card`] — deferred, the existing deferral's "unreachable with today's four message shapes" was written against the old alignment and was not revisited
- [x] [Review][Defer] P9's accepted failure mode is worse in combination with P1: on `SetTimer` failure the card never fades and is click-through, so the click cannot dismiss it, and the new-recording escape is exactly the path the patch finding above shows does not repaint [`src-tauri/src/native_preview.rs::preview_wnd_proc` `WM_PREVIEW_SET_MESSAGE`] — deferred, no reachable trigger; revisit once the `STATE_RECORDING` patch lands
- [x] [Review][Defer] A repeated identical message re-arms the hold — `s.message_at = Some(Instant::now())` on every `Some(m)` with no equality check, and P1 removed the message-less event that used to clear it [`src-tauri/src/native_preview.rs::preview_wnd_proc` `WM_PREVIEW_SET_MESSAGE`] — deferred, no producer emits the same message on a cadence today
- [x] [Review][Defer] Nothing binds `lib::run` to `warn_all` — P8's inversion was performed inside `overlay_message::warnings`, so reverting only the `lib.rs` hunk back to the per-warning loop leaves all 705 tests green [`src-tauri/src/lib.rs::run`, the `config_warnings` block] — deferred, the boot path has no test seam
- [x] [Review][Defer] `spec_clipboard_write_failure_keeps_the_degraded_pill_label` is near-vacuous — it asserts a constant against its own literal and re-asserts the card tone the preceding test already covers, exercising neither `lib::emit_pipeline_state` nor `TerminalKind::Degraded`; rewiring the pill to key off the card's tone leaves it green [`src-tauri/src/overlay_message.rs::tests`] — deferred, the record already names GATE-4 as the real proof
- [x] [Review][Defer] `MSG_CHIP_H_FRACTION`'s correctness depends on `cause_h` being exactly one line box computed with `MSG_LINE_MULT` — an invariant held by a comment, not by the code; a wrapped multi-line cause would scale the chip to the whole block [`src-tauri/src/native_preview.rs::render_message_card`] — deferred, today's cause line is single-line by construction
- [x] [Review][Defer] `PILL_HEIGHT_LOGICAL = 36.0` duplicates `native_pill::PILL_H` under a "must match" comment with no `const_assert` and no test, in two files that compile only on Windows — the same commit establishes a real cross-module call (`load_embedded_geist`), so the constant was reachable [`src-tauri/src/native_preview.rs`] — deferred, verified equal in today's tree (both `36.0`); hardening only
- [x] [Review][Defer] A font-registration failure is unobservable — `load_embedded_font` discards `AddFontMemResourceEx`'s return, so the `Once` is consumed either way and GDI silently substitutes a face for the one surface whose purpose is to work when other things are broken [`src-tauri/src/native_pill.rs::load_embedded_geist`] — deferred, named in the round's own coverage statement; needs a fallback face, not just a log line

**Dismissed as noise (14).** `pw`/`ph` are never stored in `WM_PREVIEW_SET_PILL_POS` (**false** — both branches call `rebuild_dibs(hwnd, s, pw, ph)`, which updates `phys_w`/`phys_h` and repositions the HWND) · P6 replaced a dead link with another dead one (**false** — `pipeline::generic_degrade_cause` exists at module level, the intra-doc link resolves) · `t.elapsed().as_millis() < MSG_HOLD_MS` is a type error (**false** — `const MSG_HOLD_MS: u128`) · the Change Log row is truncated mid-word (**false** — the row is complete in the committed file) · the committed `Status: in-progress` contradicts the File List (**false today** — `c8d9655` flipped story header and `sprint-status.yaml` back to `review`; they are in sync) · the `cargo build --lib` gate row states "no new warnings" about code it did not compile (the row is qualified three bullets later, and `c8d9655` added the Windows-build row that does cover those files) · D2 makes the `EmptyText` conflation worse (**false** — both the old and the new wording say "clipboard write failed", and the new *History* half is actually true on that route; the conflation stays the earlier round's untouched deferral) · P7 silently repaired a measure-vs-draw sub-pixel mismatch without claiming it (an improvement, not a defect) · `spec_boot_config_warnings_travel_as_one_event`'s `warning == card.cause.text()` assert is circular (true of that one line; the test's substantive assertions do pin the join) · `user_facing_error_text` is not applied to `warning()` / `DegradeCause::Generic` (D3's scope is error *tokens*; those paths carry prose — the real problem there is the German string, filed as a deferral) · `scale == 0` / `SPI_GETWORKAREA` failure make the geometry NaN (theoretical, pre-existing, no reachable path) · `rebuild_dibs`'s error is ignored after `win_y`/`below_pill` were committed (pre-existing pattern, unchanged shape) · whitespace-only and unbounded warning lists (P8 strictly improved this; a cap is hardening) · `pipeline.rs`'s surviving "(decision D2)" now collides with Andi's D2 (7-9's D2 is unambiguous in context).

**Machine gates re-run by this review, not taken from the record:** `cargo test --manifest-path src-tauri/Cargo.toml --lib` → **705 passed, 0 failed**, exit 0 (matches; the +4 are the three new `overlay_message` specs plus `hotkey::tests::spec_boot_config_warnings_travel_as_one_event`). `cargo build --lib` → **13 warnings** (matches). Trap #5 re-executed on `d778d5c^..d778d5c` → **0 hits** for `.emit(` / `klarvo://` (matches). Every added `_bmad-output/` line scanned for `*.rs:NNN` anchors → **0 hits**, the project-context symbol rule holds (matches). `native_pill::PILL_W`/`PILL_H` = `200.0`/`36.0` = `PILL_WIDTH_LOGICAL`/`PILL_HEIGHT_LOGICAL` (verified, not claimed). `src-tauri/fonts/` contains exactly three Geist faces and nothing mono, so P4's corrected comment is factually true (verified). Story `Status:` and `sprint-status.yaml` both read `review` today (verified). `git status` clean.

**Not verified by this review:** `native_pill.rs` and `native_preview.rs` are `cfg(target_os = "windows")` with no test module — **the decision item and four of the five patch findings live in code that nothing on this host executes.** They were reached by reading the source and tracing the producer/consumer graph. The Windows release build on `d778d5c` (exit 0) proves those files *compile*; it says nothing about the flip, the fade, hit-testing, the stale-card repaint or a single pixel. No Gradle run (correct — `git diff --stat d778d5c^..d778d5c -- android/ test-fixtures/` is 0 lines), no puppeteer re-run, no Android device, no pixels. **Andi's GATE-4 remains outstanding and nothing here substitutes for it.**

### Review Findings (bmad-code-review FINAL RE-REVIEW of the AC8 cycle, 2026-09-14, range `6cedbb5..HEAD`, fix commit `0eedb40`)

**Narrow mandate, stated up front.** This round verifies ONLY that the five items Andi scoped for fix round 2
(directive **DN1** + confirmed items **1, 2, 3, 5** of the `d778d5c` re-review) are resolved, and that the lines
`0eedb40` touched regressed nothing. **No fresh full adversarial sweep was run**; anything found outside that scope is
recorded below as residual, not acted on. Item 4 (canon / MANIFEST / mock, conductor commit `b67e297`) was explicitly
out of scope and is **not** re-verified here. Three layers ran, none failed. `0eedb40` is the last commit that touches
code — `f9bdbac` is docs + the Windows-build exit file, `9fd9ba4` is seat-costs
(`git diff --stat 0eedb40 HEAD -- src-tauri/ src/ android/ test-fixtures/ docs/` → 0 lines).

**Verification verdict on the five scoped items: items 1, 2 and 5 resolved clean; DN1 and item 3 resolved-with-a-problem.**
DN1's threshold is a genuine card height and the directive's worked example now behaves as Andi asked — but the
constant under-measures the card it is named for. Item 3's dead row and test are correct; its replacement provenance
sentence names the wrong producer symbol, i.e. the same defect class item 3 was raised to fix.
**1 decision-needed · 3 patch · 8 deferred · 20 dismissed as noise.**

- [x] [Review][Decision] (residual, budget exhausted → backlog; option (a) recommended) **DN1's threshold is a four-line *lower bound*, not "one card height" — so a reachable band of pill positions now clips the card where it used to flip.** `native_preview::MIN_CARD_H_LOGICAL` budgets exactly one line per row, but `render_message_card` wraps: `cause_lines` / `next_lines` / `hint_lines` each contribute `n × line_h`. With `preview_panel_form: "compact"` the card is `w_base = 260` logical px wide, so the text area is `260 − 2·OUTER_INSET − 2·MSG_PAD_LR` = **228** logical px, and `overlay_message::CLIPBOARD_NEXT` ("Raw text is in the clipboard · Ctrl+V to paste", 45 chars at `MSG_NEXT_PX = 12`) does not fit on one line — the `ModelNotFound` card is then ≈144 px, not 126.7. `w_logical` additionally scales by `k = font_px / BASE_FONT_PX` while the `MSG_*` sizes stay fixed, so a smaller preview font narrows the card further. Consequence: for `avail_above` in roughly **(126.7, actual_card_h)** the window stays above and `h_max_logical` clamps it, clipping the bottom line — where pre-DN1 it flipped and showed the card whole. This is inside Andi's directive as literally worded (*"die Höhe einer **vierzeiligen** Karte"*) and it is the trade option (a) buys, so it is **not** a DN1 violation — but it is his own directive's intent that decides whether the band is acceptable. Options: (a) size the constant to the widest wrapped four-line case in the narrowest supported form (`compact`, `k < 1`) and accept a threshold that flips slightly earlier; (b) keep today's bound, accept the clipping band, and say so in the record instead of claiming the card fits; (c) pass the content's required height into `compute_preview_geometry` — rejected by the code's own constraint that the geometry pass runs *before any font exists*, so it cannot measure. **Note the estimate's honesty limit:** the wrap is derived from constants and glyph-advance arithmetic, not measured — `native_preview.rs` does not compile or run on this host. Andi's call, because it re-touches DN1 [`src-tauri/src/native_preview.rs::MIN_CARD_H_LOGICAL` + `compute_preview_geometry`]

- [x] [Review][Patch] (editorial close-out, conductor) **Item 3's replacement provenance comment names a symbol that does not contain the gate — the same false-provenance defect item 3 exists to fix.** `ERROR_TOKEN_TEXT`'s new KDoc says "*`pipeline::process_audio`'s command-mode gate emits `PipelineEvent::error("feature_requires_license:CommandMode")`*", and the story's rewritten D3 resolution row repeats "*The only producer that turns such a token into an `OverlayMessage` is `pipeline::process_audio`'s command-mode gate*". Verified against today's tree: the emit is at `pipeline.rs:952`, inside **`pipeline::start_command_mode`** (`pub async fn` at `pipeline.rs:940`); `process_audio` (`pipeline.rs:1281`) contains no `CommandMode` reference at all (`grep -n CommandMode src-tauri/src/pipeline.rs` → 949, 952 only, both inside `start_command_mode`). The *conclusion* is right — `PipelineEvent::error` is the only constructor of `OverlayMessage::error`, and `start_command_mode` is its only token-bearing caller, so the one-row table is correct — it is the symbol that is wrong, inherited verbatim from the re-review's own finding text. Fix: name `pipeline::start_command_mode`'s license gate in both places [`src-tauri/src/overlay_message.rs::ERROR_TOKEN_TEXT` doc comment + story Dev Agent Record, D3 resolution row]
- [x] [Review][Patch] (editorial close-out, conductor) **`MIN_CARD_H_LOGICAL`'s docstring claims a superlative the code contradicts, and the record's omission list is short by one.** The KDoc calls it "*the tallest shape any of today's cards takes*" and says the sum "*cannot drift from the card it measures*"; both are false in the presence of wrapping (see the decision item). The record's coverage bullet names only `msg_line_h`'s rounding and the accessibility `text_scale` as the deliberate omissions — **wrapping is the third and largest one and is not listed**. Fix: state the constant as a four-line, one-line-per-row *lower bound*, drop "cannot drift" or qualify it to "cannot drift from the card's *constants*", and add wrapping to the record's omission list [`src-tauri/src/native_preview.rs::MIN_CARD_H_LOGICAL` KDoc + story Completion Notes, the `MIN_CARD_H_LOGICAL` coverage bullet]
- [x] [Review][Patch] (editorial close-out, conductor) **Record: "the pre-DN1 clamping behaviour" is a false comparison.** The new coverage bullet reassures that at 225 % text scale "*a pill position with ~130 px above it will keep the card above and let it be clamped — the pre-DN1 clamping behaviour, not a new defect*". Pre-DN1 (i.e. at `d778d5c`) `avail_above = 130 < h_wanted = 600` made the window **flip below the pill**; it did not clamp above. The clamping shape being invoked is the pre-**D1** behaviour. The outcome described is indeed benign, but the sentence that licenses it is wrong — and this is the story's own DN1 coverage statement [story Completion Notes, the `MIN_CARD_H_LOGICAL` coverage bullet]

- [x] [Review][Defer] A poisoned `history_db` mutex is silent — `if let Ok(db) = state.history_db.lock()` has no `else` branch and no log line, while item 2 has just made that outcome decide the card's wording [`src-tauri/src/pipeline.rs::stop_and_process_pipeline`, the history block] — deferred, pre-existing shape; item 2 did not require a log and the card does now tell the user
- [x] [Review][Defer] Nothing tests that the **shell** passes the real history outcome — every new spec drives `terminal_degrade_cause` as a pure function with hand-supplied booleans; hardcoding `history_saved = true` at the call site leaves all 707 green, restoring the false promise in full. The new test's docstring ("*this pins that it is the thing the cause carries*") over-reads what it pins [`src-tauri/src/pipeline.rs::tests::spec_history_outcome_decides_whether_the_card_names_history`] — deferred, disclosed in the record's coverage statement; `stop_and_process_pipeline` has no test seam
- [x] [Review][Defer] At 225 % accessibility text scale a four-line card exceeds 126.7 logical px, so it stays above and is clamped [`src-tauri/src/native_preview.rs::MIN_CARD_H_LOGICAL` vs `msg_line_h`] — deferred, **named in the record** (the sentence that names it is patch finding 3 above)
- [x] [Review][Defer] `STATE_RECORDING` arriving with `had_card == false` while the window is visible from a prior run's live text is not hidden — the new hide is gated on `had_card` [`src-tauri/src/native_preview.rs::preview_wnd_proc`, `WM_PREVIEW_SET_STATE`] — deferred, needs a `STATE_RECORDING → STATE_RECORDING` sequence with no terminal state in between; the pre-DN1 code had the identical hole
- [x] [Review][Defer] `MIN_CARD_H_LOGICAL` is a hand-transcribed eleven-term sum with no `const_assert` and no test binding it to `render_message_card`; adding a row or a padding to the card drifts it silently [`src-tauri/src/native_preview.rs`] — deferred, same family as the already-deferred `PILL_HEIGHT_LOGICAL` "must match" comment; hardening
- [x] [Review][Defer] The Turso `push_single_entry` spawn is gated on `cfg_for_history` + non-empty url/token, **not** on `history_saved`, and carries `raw_text` — so `in_history: false` can say "the raw text could not be saved" while a remote copy went out [`src-tauri/src/pipeline.rs::stop_and_process_pipeline`, the auto-sync block] — deferred, it under-promises rather than over-promises, and the local History list the card points at genuinely has nothing
- [x] [Review][Defer] The `in_history: false` card ships `next: None, hint: None` while `terminal_degrade_cause`'s own docstring says the transcript stays in `Klarvo.log` — the one case where the text is genuinely lost is the one card offering no recovery path [`src-tauri/src/overlay_message.rs::DegradeCause::card`] — deferred, AC8 does not require a hint on this variant
- [x] [Review][Defer] Both sides tiny → `phys_h.max(1)` with no minimum-height floor [`src-tauri/src/native_preview.rs::compute_preview_geometry`] — deferred, **unchanged re-statement of the previous round's own deferral**; DN1 did not touch it

**Dismissed as noise (20).** Moving the `terminal_degrade_cause` call ~190 lines later is unverified for intervening readers (**false** — `degrade_cause` is read nowhere between the old and new call sites and there is no early `return` in that span; verified line-by-line) · the serialized shape of `DegradeCause` changed and no frontend consumer was updated (**false** — `DegradeCause` derives no `Serialize`, it rides on `PipelineEvent::message` which is `#[serde(skip_serializing)]`, and `grep -rn "ClipboardWriteFailed\|degradeCause" src/` is empty) · `Ok(_) => history_saved = true` may be true without a row (**false** — plain `INSERT`, no `OR IGNORE`, `Ok(conn.last_insert_rowid())`) · `text_buffer` is not cleared before re-arming (**false** — `s.armed = true; s.text_buffer.clear();` in the same branch) · `MIN_CARD_H_LOGICAL`'s `2·OUTER_INSET` double-counts the `12.0` in `avail_above`/`avail_below` (**false** — the `12.0` is a work-area edge margin, also applied to `work_left`/`work_right`; `OUTER_INSET` is inside the window's own DIB) · the new `const` expression is type-checked by no named gate (the operands are all `f32`, and the Windows release build on `0eedb40` compiled the file, exit 0 — the commit names that gate) · the constant's name and doc assert opposite things (`MIN_` is the minimum room the card needs, which the tallest card sets — consistent; the superlative itself is a patch finding) · keep the `OfflineMode` row as a harmless fallback (contradicts item 3's own confirmed directive) · the retargeted `OfflineMode` assertion pins dead code (it discriminates, and the test's new docstring says why `Sync` and `OfflineMode` both belong there) · the `spec_progress_events_carry_no_card` dismissal list is incomplete because the `else` arm also replaces the card (**false** — the `else` arm explicitly *preserves* the card when `s.message.is_some()`) · cross-module lore parked in a test docstring (item 5b asked for exactly that correction; all three named routes — `STATE_RECORDING`, `WM_TIMER`, `WM_LBUTTONDOWN` — verified to exist) · hoist the two new cause strings into `const`s (one consumer each; the project rule is factor out on ≥2 real consumers, which is why `CLIPBOARD_NEXT` is a const and these are not) · the hand-computed "≈126.7"/"≈127" figures in prose are unguarded (both independently re-derived to **126.70**; correct) · the eight-name `require_license!` enumeration will go stale (verified exactly right against today's call sites; hardening at most) · the threshold is card-shaped but also governs the live preview (**Andi's directive** — *"gleiches gilt für die Live-Preview in dieser Lage"*; option (c) two thresholds was rejected in the decision this round implements) · flipping into a box that is still too small (the `avail_below > avail_above` guard makes the flip a strict improvement, and the docstring states it) · the `126.7..127.0` band loses a px to `as i32` (sub-pixel, theoretical) · a prior run's terminal message posted after the new run's recording state (posts are FIFO on one queue and the pipeline is sequential; no producer ordering was shown) · `paste_failed` true while `degrade_cause` is `None` leaves the pill claiming "In Clipboard" (pre-existing, pinned by `spec_non_degraded_run_never_gains_a_cause`'s docstring and **already in `docs/backlog.md`** under "Story 7-10 residuals") · `header_h.max(MSG_DOT_SIZE·sc)` is elided from the constant (harmless — `MSG_DOT_SIZE = 7.0` < `14.5`).

**Machine gates re-run by this review, not taken from the record:** `cargo test --manifest-path src-tauri/Cargo.toml --lib` → **707 passed, 0 failed**, exit 0 (matches the record's 705 → 707). `cargo build --lib` → **13 warnings** plus the summary line, every one pre-existing and none in a touched file (matches). Trap #5 re-executed on `0eedb40` → **0 hits** for `.emit(` / `klarvo://` (matches). Every added `_bmad-output/` line scanned for `*.rs:NNN` anchors → **0 hits**, the project-context symbol rule holds (matches). `MIN_CARD_H_LOGICAL` re-derived independently from the eleven source constants → **126.70** logical px (matches "≈ 126.7"). DN1's worked example re-derived from `compute_preview_geometry`: pill `y = 400`, 1080p, scale 1 → `avail_above = 380`, `380 < 126.7` false → **stays above**, as the directive asks; pill `y = 20` → `avail_above = 0` → **flips**, the top-edge case D1 was raised for. `ClipboardWriteFailed` has exactly **one** production construction site (`pipeline::terminal_degrade_cause`); `terminal_degrade_cause` has exactly one production call site (`pipeline.rs:2147`). `overlay_message` carries **11** spec fns, matching the record's "10 → 11". Story `Status:` and `sprint-status.yaml` both read `review` (verified). `git status` clean before this review wrote its findings.

**Not verified by this review:** `native_preview.rs` is `cfg(target_os = "windows")` with no test module — **the decision item and two of the three patch findings live in code that nothing on this host compiles or executes.** They were reached by reading the source and tracing the producer/consumer graph; the `compact`-form wrap in the decision item is glyph-advance *arithmetic*, not a measurement. The Windows release build on `0eedb40` (exit 0) proves those files compile — and its one new warning (`native_preview.rs`'s unused `ShowWindow` `BOOL`, disclosed by name in `f9bdbac`) is consistent with every other `ShowWindow` in the file. Nobody has seen the flip, the hide, the clamp or a pixel. No Gradle run (correct — `git diff --stat 0eedb40^..0eedb40 -- android/ test-fixtures/` is 0 lines), no puppeteer re-run, no Android device. **Andi's GATE-4 remains outstanding and nothing here substitutes for it.**
...
| D1 | The card has no surface when the pill sits near the top (decision) | Andi's directive: *"Wenn oben weniger Platz als die Kartenhöhe ist, erscheint die Karte 8 px UNTER der Pill; gleiches gilt für die Live-Preview in dieser Lage."* `native_preview::compute_preview_geometry` gains `work_bottom` and returns a 5th value `below_pill`: when the room above the pill is less than the card wants, the window is placed `GAP_LOGICAL` (8 px) **below** the pill, sized to the room there. ~~(`BASE_MAX_HEIGHT × k`)~~ **Corrected in the AC8 fix round 2 (DN1):** "what the card wants" was implemented as `h_wanted`, the *window's* 600 px upper bound, which flipped the whole window — live preview included — for roughly the upper half of the screen. It is now `MIN_CARD_H_LOGICAL`, one four-line card; see the DN1 row below. New `PILL_HEIGHT_LOGICAL` (36, mirroring `native_pill::PILL_H`). `PreviewWindowState.below_pill` flips **both** renderers from bottom-aligned to top-aligned (`render_message_card` and `render_frame` — the live preview follows, as the directive says). Recomputed on every `WM_PREVIEW_SET_PILL_POS`, i.e. on every pill drag. **One arithmetic guard, named because it is mine and not Andi's:** the flip is skipped when it would yield *less* height than staying above (`avail_below > avail_above`), since moving the card into an even smaller box would defeat the directive it implements. |
...
| D3 | Machine tokens get a prominent, untruncated card (decision) | Andi's directive: new `overlay_message::ERROR_TOKEN_TEXT` and `user_facing_error_text`, applied inside `OverlayMessage::error`. `feature_requires_license:CommandMode` → `Command mode needs a license`. **Unknown tokens stay verbatim** — the raw string is what matches `Klarvo.log` and what a support answer needs; guessing is worse. `PipelineEvent::error` keeps the raw token in its own `error` field, so **the frontend payload and every consumer of `event.error` are unchanged** — only the card is translated. ~~The two rows are the `feature_requires_license:*` tokens that exist in today's tree (`grep -rn feature_requires_license src-tauri/src/` → `lib.rs`'s `require_license!`, `pipeline.rs` command-mode gate, `commands/whisper.rs` ×2).~~ **Corrected in the AC8 fix round 2 (re-review item 3):** that grep summary conflated "the token exists somewhere" with "the token can reach a card", and the row it justified was dead. The table is **one** row. `require_license!` (`lib.rs`) formats the same prefix but only inside Tauri commands and only for `WhisperMode` · `AppProfiles` · `Snippets` · `Sync` · `UnlimitedHistory` · `FillerAnalysis` · `VoiceNotes` · `AlternativeProviders` — **neither** mapped token, and an `Err(String)` to the frontend either way. `commands/whisper.rs`'s `OfflineMode` is the same shape, so its row translated a string no card ever receives; it is removed. The only producer that turns such a token into an `OverlayMessage` is `pipeline::process_audio`'s command-mode gate, i.e. `CommandMode`. |
...
**AC8 RE-REVIEW — FIX ROUND 2, the FINAL round of the AC8 cycle (2026-09-14, Linux host, no device).**
Scope fixed by Andi: the confirmed items **1, 2, 3, 5** of the `d778d5c` re-review plus directive **DN1** on its
one decision item. **Item 4 (canon / MANIFEST / mock) was already carried by the conductor in `b67e297`** and was
not re-touched. Nothing from the re-review's 12 deferrals or 14 dismissals is in this round.

| Gate | Command | Result |
|---|---|---|
| Rust unit suite | `cargo test --manifest-path src-tauri/Cargo.toml --lib` | `test result: ok. 707 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out` (705 → **707**, +2 new specs) |
| Rust build | `cargo build --manifest-path src-tauri/Cargo.toml --lib` | `Finished`; **13 warnings, all pre-existing** — the same set as the previous round, none added |
| `MIN_CARD_H_LOGICAL` const-eval | standalone `rustc -O` on the constant + the re-review's worked example | **`126.70000267028809`** logical px. `cargo` cannot check this — `native_preview.rs` is stripped to an empty crate on Linux — so the `f32 → f64` casts in a `const` context and the value were compiled and printed in a throwaway program, then deleted. Flip outcomes: pill at `y = 400` (the re-review's example, `avail_above = 380`) → **stays above** (it flipped below before); pill at `y = 20` (`avail_above = 0`, D1's top-edge case) → **flips below**; `avail_above = 120` → **flips below**. |
| Parse check, Windows-gated files | `rustc --edition 2021 --crate-type lib --emit=metadata src/{native_preview,native_pill}.rs` | exit **0**. **Parsing only, and stated as such**: the inner `#![cfg(target_os = "windows")]` empties both files after the parser has run. Verified non-vacuous in this session — a deliberately broken copy of `native_preview.rs` exits 1 with 2 errors. |
| Trap #5 | `git diff -U0 -- src-tauri/src \| grep -cE '^\+.*(\.emit\(\|klarvo://)'` | **0** — no new event name, no new payload field, no new emit. `DegradeCause` is `#[serde(skip_serializing)]` cargo on `PipelineEvent::message`, so its new field does not reach the wire either. |
| Kotlin JVM gate | — | **NOT RUN.** Desktop-only round: `git diff --stat -- android/ test-fixtures/ src/` returns **0 lines**. A scope statement, not a green. |
| Frontend | — | **NOT RUN and not needed.** No file under `src/` changed; `status_line()` is byte-identical on both `ClipboardWriteFailed` shapes, which a spec now pins, so the D1 proxy smoke has nothing new to see. |
| Lint | `cargo clippy` | **blocked** — still not installed for the toolchain; not installed around (project-context). Unchanged from every previous round. |
| **Windows release build** | `scripts/windows-build.sh` on `0eedb40` | **exit 0 — `FERTIG. klarvo.exe enthält 0eedb40`**, exe written `2026-09-14 13:18:33`, the script's own freshness check passed (exit 3 would have caught a stale exe). Installer bundled + `rsign` signature verified. Artifacts: `gate4-evidence/7-10/windows-build-r4.{log,exit}`. **This is the ONLY gate that compiles `native_preview.rs` at all** — it proves `MIN_CARD_H_LOGICAL`'s const-eval in place, the types, the borrows and the Win32 signatures of the new `ShowWindow` call. It proves **nothing** about the flip, the hide, or a pixel. |
| ⚠ Windows warning count | `grep -c 'unused \`BOOL\`'` on the build log | **75 warnings, up from 74 on `d778d5c` — this round adds exactly one, and it is mine.** `unused \`BOOL\`` goes 56 → **57**; the new one is `native_preview.rs:1550`, the `ShowWindow(hwnd, SW_HIDE);` of item 1. It is left unprefixed **deliberately**: the line it mirrors 13 lines below (the terminal branch's own `SW_HIDE`) is written exactly the same way, and every other `ShowWindow` in the file warns identically. Matching the surrounding code (project-context) was chosen over a lone `let _ =`; flagged here rather than buried, because "no new warnings" would have been false. |
| Scope check | `git status` after both inversions | clean — no inversion residue |

**Inversions (AC6 discipline).** Only item 2 changes behaviour a Linux host can observe; both of its halves were
shown RED and reverted. **DN1, item 1 and item 5a have no inversion and can have none** — they live in
`native_preview.rs`, `#[cfg(target_os = "windows")]`, not compiled here. **Item 3 and item 5b are a comment, a
dead table row and a docstring**; the dropped row's test assertion was rewritten to assert the *verbatim*
fallback instead, which is a real assertion about the shipped table, not an inversion.

| # | Item | Reverted change (symbol) | Test(s) that went RED | Verbatim failure | Discriminating? |
|---|---|---|---|---|---|
| 1 | item 2 (card) | `overlay_message::DegradeCause::card`, `ClipboardWriteFailed` arm — the `in_history` branch replaced by the unconditional "raw text is in History" | `overlay_message::tests::spec_clipboard_write_failure_only_names_history_when_the_write_succeeded`, `pipeline::tests::spec_history_outcome_decides_whether_the_card_names_history` | `705 passed; 2 failed` | **Yes.** Both failures are the `in_history: false` half; the `in_history: true` assertions in the same two tests stay green, so the failure isolates the *conditional* from the wording. A blanket rewrite of the line would fail the true half too. |
| 2 | item 2 (threading) | `pipeline::terminal_degrade_cause` — `in_history: history_saved` hardcoded back to `in_history: true` | `pipeline::tests::spec_history_outcome_decides_whether_the_card_names_history` | `706 passed; 1 failed`; `left: ClipboardWriteFailed { in_history: true }` | **Yes.** The card test stays green (the card itself is still correct), so the failure isolates the *threading* from the wording — the two halves of item 2 fail independently. |

**Resolution rows — written from `git diff`, anchored at `file::symbol`:**

| # | Finding | What changed |
|---|---|---|
| DN1 | The flip threshold is the 600 px window cap, not a card height (decision) | Andi's directive verbatim: *"Die Flip-Schwelle ist EINE Kartenhöhe (`MIN_CARD_H_LOGICAL`, die Höhe einer vierzeiligen Karte), nicht die Fenster-Obergrenze von 600 px. Die Karte und die Live-Preview kippen nur dann unter die Pill, wenn oben weniger als diese Kartenhöhe frei ist."* → option (a). New `native_preview::MIN_CARD_H_LOGICAL`, and `compute_preview_geometry`'s `below_pill` compares against it instead of `h_wanted`. The constant is **summed from the constants `render_message_card` lays out with** (`OUTER_INSET`, `MSG_HEAD_PAD_TB`, `MSG_HEADER_PX`, `MSG_BORDER_W`, `MSG_BODY_PAD_TOP/BOTTOM`, `MSG_CAUSE_PX`, `MSG_LINE_GAP` ×2, `MSG_NEXT_PX`, `MSG_HINT_PX`, `MSG_LINE_MULT`) rather than written as a magic number, so it cannot drift from the card it measures; `msg_line_h`'s rounding and the accessibility text scale are deliberately outside it, because this is a placement threshold evaluated before any font exists. `h_wanted` still caps the window's *height* — only the *side* decision moved. One line changed in the function; the doc comment now states the substitution and why the first version was wrong. |
| 1 | The `STATE_RECORDING` branch dismisses the card but neither repaints nor hides | `native_preview::preview_wnd_proc`, `WM_PREVIEW_SET_STATE`'s `STATE_RECORDING` arm: it captures `s.message.is_some()` **before** `dismiss_message`, and on a card that was up it now calls `ShowWindow(hwnd, SW_HIDE)` + `s.was_visible = false` — mirroring the terminal branch's own message-less path. `dismiss_message` only ever cleared state; the hiding used to be done by the `set_message(None)` that P1 turned into a no-op inside the 4 s hold, which is why P1 re-opened a deferral it had itself made unreachable. `render_frame` would reach the same `SW_HIDE` through its `!armed \|\| text_buffer.is_empty()` guard — the explicit call states it without a measure-and-draw pass. Restores AC8's "a new recording … dismisses it at once", which with `live_preview_enabled: false` was broken for the whole record → STT → cleanup cycle. |
| 2 | "raw text is in History" asserted while the History write is best-effort | Three symbols. (a) `overlay_message::DegradeCause::ClipboardWriteFailed` becomes `ClipboardWriteFailed { in_history: bool }`; its `card()` reads `Clipboard write failed — raw text is in History` only when the write landed, and `Clipboard write failed — the raw text could not be saved` when it did not. Tone `Error`, header `TEXT LOST` and `next: None` are unchanged on both. (b) `status_line()` matches `{ .. }` — it names no surface, so it stays **byte-identical** on both shapes and D1's status line is untouched; a spec asserts the two are equal. (c) `pipeline::terminal_degrade_cause` gains a `history_saved` parameter, and the call **moved** from just after `deliver_text` to just before the `done_with_clipboard_only` emit, i.e. after the history block — which is where the outcome first exists. The history block's `if let Err(e) = add_entry` became a `match` setting `history_saved = true` on `Ok`; the poisoned-mutex path leaves it `false`, as does a swallowed `add_entry` error. Same false-promise shape review round 1 (F6) removed from the clipboard half of this very variant. |
| 3 | `ERROR_TOKEN_TEXT`'s provenance comment is false; half the table is dead | `overlay_message::ERROR_TOKEN_TEXT` drops the `feature_requires_license:OfflineMode` row and the doc comment is rewritten from provenance-by-grep to provenance-by-producer: `pipeline::start_command_mode`'s licence gate (corrected in the close-out; the fix-round row said `process_audio`) is the **only** site that turns such a token into an `OverlayMessage`, so `CommandMode` is the whole table; `require_license!` (8 features, none of them mapped) and `commands/whisper.rs` return `Err(String)` to the frontend and never reach a card. `spec_known_error_tokens_are_translated_unknown_ones_are_verbatim` keeps its `OfflineMode` line but flips it to assert the **verbatim** fallback, and its docstring now says why `Sync` is there — it is a real token in the tree that still never reaches a card, which is exactly the case the fallback must survive. The story's D3 resolution row is corrected in place, struck through rather than deleted (Epic-7 retro D2). |
| 5 | Two comments falsified by P1 in the commit that wrote it | (a) `native_preview::preview_wnd_proc`, the `STATE_RECORDING` comment: "*Its own `set_message(None)` arrives just before this, so this is the belt to that braces*" is replaced by what is true after P1 — that this arm is now the only thing dismissing a card on the new-recording route, and why the hide below it is needed. (b) `hotkey::tests`, the docstring above `spec_progress_events_carry_no_card`: "*the overlay clears any card still on screen when the next run starts*" said the opposite of P1's behaviour; it now states that a message-less event does **not** take a live card down and names the three things that do. |

**Coverage statement — AC8 fix round 2. What 707 does NOT cover.** *(project-context: "a number states what it covers".)*

- **707 Rust green proves the message model and the cause-threading — nothing about the card on screen.** The 2 new
  specs decide: that the `ClipboardWriteFailed` card names History **only** when the History write succeeded, that
  its one-line form is unchanged either way, and that `terminal_degrade_cause` carries the run's actual outcome.
  Claim proved: **logic and wording**. Not layout, not a pixel.
- **Item 2's shell wiring is NOT exercised by any test.** What the specs drive is
  `terminal_degrade_cause(cause, paste_failed, history_saved)` as a pure function. That
  `stop_and_process_pipeline` computes `history_saved` from the real `history::add_entry` call, that the moved call
  site still sees the same `degrade_cause`, and that the poisoned-mutex path leaves it `false` were read from the
  source — `stop_and_process_pipeline` needs an `AppHandle` and has no seam. The `FailingHandler`-with-`copy_only`
  gap the re-review deferred is still open and still unclosed here.
- **DN1, item 1 and item 5a have ZERO test coverage and always will on this host.** They are in
  `native_preview.rs`, `#[cfg(target_os = "windows")]` with no test module, which `cargo test --lib` does not
  compile. The Windows release build on `0eedb40` (exit 0) closes exactly one half of that hole — it proves they
  *compile*, with `MIN_CARD_H_LOGICAL`'s const-eval, types, borrows and Win32 signatures checked in place. Not
  exercised by anything, anywhere: that the window actually stays **above** a pill at mid-screen after DN1, that it
  still flips at the top edge, the near-equal band, the live preview's placement in either case, and that a stale
  card really disappears the instant a new recording starts with live preview off. **That is Andi's GATE-4.**
- **`MIN_CARD_H_LOGICAL` is a threshold, not a measurement of the card that will be drawn.** It omits
  `msg_line_h`'s per-line rounding, the accessibility `text_scale` **and line wrapping** (compact form: the clipboard
  line wraps, a `ModelNotFound` card is ≈144 px, not 126.7). So a pill position with ~130–144 px above it keeps the
  card above and lets it be clamped — the pre-**D1** behaviour (before D1 the window never flipped; before DN1 it
  flipped for that band), not a new defect, but it is not "the card always fits" either. Residual: final re-review
  decision item (size the constant to the widest wrapped case in the compact form) — not applied, budget exhausted.
- **Android: not touched, not re-run.** `git diff --stat -- android/ test-fixtures/` → 0 lines. No claim is made
  about the Kotlin suites beyond "no Kotlin file changed". S6 held.
- **Frontend: not touched, not re-run.** `git diff --stat -- src/` → 0 lines, and the status-line wording is pinned
  byte-identical by a spec. The D1 proxy smoke's 13/13 stands from the previous round and is **not** re-claimed
  as this round's evidence.
- **Item 4 of the re-review (canon / MANIFEST / mock) is NOT this round's work** — the conductor carried it in
  `b67e297`; nothing here re-verifies it.
- **The re-review's 12 deferred and 14 dismissed findings were not touched** and are not covered by anything here.
...
**Added — AC8 fix round 2**
- `_bmad-output/implementation-artifacts/gate4-evidence/7-10/windows-build-r4.exit` — the Windows release build's
  exit code on `0eedb40` (**0**). `windows-build-r4.log` is written next to it but **not committed** —
  `.gitignore:15` (`*.log`), as with r2 and r3. The numbers quoted above (75 warnings, 57 `unused BOOL`, +1 each
  against build r3) were counted from the file on disk.
...
**Modified — AC8 fix round 2 (DN1 + re-review items 1, 2, 3, 5)**
- `src-tauri/src/native_preview.rs` — new `MIN_CARD_H_LOGICAL` (summed from the card's own layout constants) and
  `compute_preview_geometry`'s `below_pill` now compares against it instead of `h_wanted`, with the doc comment
  stating the substitution (DN1); `preview_wnd_proc`'s `WM_PREVIEW_SET_STATE` `STATE_RECORDING` arm captures
  `message.is_some()` before `dismiss_message` and hides the window when a card was up (item 1), its comment
  rewritten (item 5a). **Not compiled on this host.**
- `src-tauri/src/overlay_message.rs` — `DegradeCause::ClipboardWriteFailed` gains `{ in_history: bool }`;
  `card()`'s cause line is conditional on it, `status_line()` matches `{ .. }` and stays byte-identical;
  `ERROR_TOKEN_TEXT` drops the dead `OfflineMode` row and its doc comment is rewritten from provenance-by-grep
  to provenance-by-producer (item 3). Tests: 3 updated (`spec_clipboard_write_failure_card_…`,
  `spec_clipboard_write_failure_keeps_the_degraded_pill_label`, `spec_status_line_wording_is_unchanged_by_the_card`),
  1 retargeted (`spec_known_error_tokens_…`, `OfflineMode` now asserts the verbatim fallback), **1 added**
  (`spec_clipboard_write_failure_only_names_history_when_the_write_succeeded`). 10 → **11** specs.
- `src-tauri/src/pipeline.rs` — `terminal_degrade_cause` gains `history_saved` and its call **moves** below the
  history block; that block's `if let Err` becomes a `match` setting a new `history_saved` flag (item 2).
  Tests: 3 updated for the new arity, **1 added**
  (`spec_history_outcome_decides_whether_the_card_names_history`).
- `src-tauri/src/hotkey/mod.rs` — the docstring above `spec_progress_events_carry_no_card` corrected (item 5b).
  Doc-only; no code line changed.
- `_bmad-output/implementation-artifacts/7-10-…md` — the re-review's items 1/2/3/5 and the decision item checked
  off, the D1 and D3 resolution rows corrected in place (struck through, not deleted), this round's Review
  Follow-ups block, gates / inversions / resolution rows / coverage statement, this File List block and the
  Change Log.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `7-10` back to `in-progress` for the fix round,
  then `review`.

**Not modified (verified):** `src-tauri/src/config.rs`, `src/components/**`, `src-tauri/src/history/mod.rs`, `test-fixtures/**`, `KlarvoAccessibilityService.performEnter`. **Round 2 additionally:** no file under `src-tauri/` and no file under `android/` (`git diff --stat` → zero such paths), and `src/App.tsx` unchanged — the fix is in the hook, not the renderer. **AC8 fix round 2 additionally:** nothing under `src/`, `android/`, `test-fixtures/` or `docs/design/` (`git diff --stat` → zero such paths) — item 4's canon/MANIFEST/mock work is the conductor's `b67e297`, not this round's; `native_pill.rs` untouched.

## Change Log

| Date | Change |
|---|---|
| 2026-09-14 | **DONE — Andi's GATE-4 round 2 GREEN on `37b2204` ("getestet, GATE-4 grün"), one tuning request: card fades too early → `MSG_HOLD_MS` 4 s → 8 s in `4f67600` (conductor, direct: a constant Andi decided, no fix worker); pill keeps 4 s. Windows build `4f67600` exit 0; Andi's go on the 8 s build. Both status fields → `done`, `epic-7` → `done`. Residual (backlog): flip-threshold band ≈127–144 px. 13 seats, ≈$144. | conductor |
| 2026-09-14 | **AC8 cycle review-cleared (conductor close-out).** GATE-4 r1 FAILED (pill truncation) → Option A (pill = status light, preview card = message surface). Dev `8b81864`, review (3 decisions D1–D3 + 11 patches), fix `d778d5c`, re-review (DN1 + 5), fix `0eedb40`, final re-review (1 residual decision → backlog + 3 editorial, fixed in `37b2204`). Windows build green on every code commit. GATE-4 round 2: `gate4-evidence/7-10/verdict.md`. Status stays `review` (both fields). | conductor |
| 2026-09-14 | **AC8 fix round 2 — the FINAL round of the AC8 cycle. Exactly the re-review's confirmed items 1, 2, 3, 5 + Andi's directive DN1; item 4 was the conductor's `b67e297`; the 12 deferrals and 14 dismissals untouched.** **DN1** — the below-pill flip threshold was `h_wanted`, the *window's* 600 px cap, which moved the whole window (live preview included) below the pill for roughly the upper screen half; it is now `MIN_CARD_H_LOGICAL` ≈ 126.7 logical px, one four-line card, summed from the card's own layout constants. The re-review's worked example (pill at `y = 400`, 380 px above) now stays **above**; the top edge still flips. **Item 1** — `WM_PREVIEW_SET_STATE`'s `STATE_RECORDING` arm now hides the window after `dismiss_message`: P1's hold guard had turned the `set_message(None)` that used to do the hiding into a no-op, so with live preview off a dead failure card covered the whole next record → STT → cleanup cycle. **Item 2** — the `TEXT LOST` card promised "raw text is in History" while that write is best-effort; `DegradeCause::ClipboardWriteFailed` gains `{ in_history }`, `terminal_degrade_cause` takes the run's actual history outcome, and its call moved below the history block. `status_line()` is byte-identical on both shapes, pinned by a spec — D1's status line untouched. **Item 3** — `ERROR_TOKEN_TEXT`'s provenance comment named `require_license!`, which produces neither mapped token; the comment is rewritten by producer, the dead `OfflineMode` row is removed, and the story's D3 row is corrected in place. **Item 5** — the two comments P1 falsified in its own commit. Gates: `cargo test --lib` 705 → **707**, `cargo build --lib` 13 pre-existing warnings, trap #5 **0 hits**, `MIN_CARD_H_LOGICAL` const-evaluated in a throwaway program (`cargo` cannot — the file is empty on Linux), **Windows release build green on `0eedb40` (exit 0, exe fresh 13:18:33)**. ⚠ That build reports **75 warnings, one more than build r3's 74** — `native_preview.rs:1550`, item 1's `ShowWindow(hwnd, SW_HIDE);`, left unprefixed to match the identical line 13 rows below it; named rather than buried. Both inversions shown RED and reverted; `git status` clean. **DN1, item 1 and item 5a are in `native_preview.rs`, `#[cfg(target_os = "windows")]` with no test module — ZERO test coverage; the Windows release build proves only that they compile. Nobody has seen the flip, the hide or a pixel. Andi's GATE-4 remains outstanding.** |
