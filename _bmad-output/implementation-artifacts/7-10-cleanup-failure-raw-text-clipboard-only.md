...
| Lint | `cargo clippy` | **blocked** — still not installed for the toolchain; not installed around (project-context). Unchanged from every previous round. |
| **Windows release build** | `scripts/windows-build.sh` | see the row added below after the run — it is the only gate that compiles `native_preview.rs` at all |
| Scope check | `git status` after both inversions | clean — no inversion residue |
...
- **DN1, item 1 and item 5a have ZERO test coverage and always will on this host.** They are in
  ...
  compile. The Windows release build below closes exactly one half of that hole — it proves they *compile*. Not
