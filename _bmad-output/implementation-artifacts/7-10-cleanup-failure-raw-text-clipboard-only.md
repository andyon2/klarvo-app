...
- **8 of the 14 items in this round have ZERO machine coverage and can have none on this host.** P1, P2, P3, P4,
  P7, P9, P10 and D1 are all in `native_preview.rs` / `native_pill.rs`, which are `#[cfg(target_os = "windows")]`
  with no test module: `cargo test --lib` does not compile them, so the 705 says nothing about them — not even
  that they compile. Only a **parse** check (`rustc --emit=metadata`, exit 0) ran, and after the parser the inner
  `#![cfg]` strips both files to an empty crate. That they compile is the Windows build's claim; that they *look*
  right is Andi's GATE-4.
