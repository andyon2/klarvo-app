...
- **`MIN_CARD_H_LOGICAL` is a threshold, not a measurement of the card that will be drawn.** It omits
  `msg_line_h`'s per-line rounding and the accessibility `text_scale`. At 225% text scale a four-line card is
  taller than 126.7 px, so a pill position with ~130 px above it will keep the card above and let it be clamped —
  the pre-DN1 clamping behaviour, not a new defect, but it is not "the card always fits" either.
