...
**Gate runs — FIX ROUND 3 (review round-3 findings RES-2 + R3-1..R3-5, R3-7), 2026-09-12,
`conductor/story-7-9`:**

- `cargo test --lib` in `src-tauri/` — **681 passed, 0 failed, 0 ignored** (unchanged count:
  this round adds no `#[test]`; RES-2 adds three *cases* to the existing fixture table and
  R3-2/R3-3/R3-7 change doc text and assertion predicates inside existing tests).
  Covers: Rust unit + inline spec tests on the **Linux** target. Does **NOT** cover the
  Windows-only arms (`llm::local`, the `local` cleanup arm, native overlays) — in particular
  REG-1's Windows branch is still not compiled here, so R3-7's *edited* message is the one that
  runs on this host and the `cfg!(target_os = "windows")` branch's message was not executed;
  also not the Tauri runtime, `save_advanced_settings` as a *command*, the real `Klarvo.log`,
  any network call, or `tests/pi_security.rs`.
- Rust warning count **unchanged**: `13` (lib) / `18` (lib test, 3 duplicates) / `1`
  (`pi_security`) — same `cargo check --lib --tests` measurement as rounds 1 and 2. No new
  warnings.
- `npm run build` (`tsc && vite build`) — **green**, `✓ built in 1.52s`. Strict `tsc`
  type-checks the R3-4/R3-5 restructure; it executes nothing.
- JVM gate `./gradlew :app:testUniversalDebugUnitTest --rerun-tasks` — **186 tests, 0 failures,
  0 errors, 0 skipped across 23 suites** (unchanged count; `cleanupModelSanitizeMatchesFixture`
  confirmed **present** in the result XML, not silently skipped). `--rerun-tasks` again
  mandatory — this round edits `test-fixtures/twin-constants-vectors.json`.
  - Run **device-free** (7-8 precedent). **Deviation from rounds 1–2, stated because it changes
    what the run proves:** the usual `rm`/`cp` sync into
    `src-tauri/gen/android/app/src/{main,test}/java/com/klarvo/voice` required an approval this
    non-interactive session cannot give, so it was **not** run. Instead the generated tree was
    first *diffed* against the tracked sources — it differed in exactly the two files this round
    changes and in nothing else — and those two files were then edited to be **byte-identical**
    to `android/kotlin-src`/`android/kotlin-test` (verified afterwards with `diff -rq` over both
    directories: no differences). So the gate ran against this round's real Kotlin sources.
    `scripts/android-smoke.sh` was **not** used — it fails on "Kein Gerät gefunden" before its
    JVM gate and no device/AVD was reachable from this host.
  - Count taken over **all 23 result XMLs** in the `testUniversalDebugUnitTest` variant dir, not
    the smoke banner's single suite; the nine stale variant dirs were excluded.
  - Covers: pure Kotlin/JVM logic. Does **NOT** cover JNI, any device or emulator, Android
    UI/rendering, or `readConfig`'s real file I/O and license gating.
- **Desktop proxy smoke re-run** (puppeteer 24.38.0 vs `npm run preview` :1422, real Chromium;
  required because `src/components/SettingsPanel.tsx` changed) — **28/28 checks passed, 0
  uncaught page errors**, unchanged from rounds 1–2. The preview server was stopped afterwards.
  - **This is a regression check only, and it is weaker than it looks for this round.** R3-4 and
    R3-5 both live on the *skip* path, which only runs when `getAdvancedSettings()` **rejects**;
    the proxy serves mocks and has no backend, so **neither the relocated report nor the removed
    auto-dismiss was ever driven**. No new check was added because the harness cannot reach that
    code. What the run proves is that the panel still renders exactly as before (Paste & Behavior
    rows, all four Advanced sections, expert mode on and off, D1's badge and input checks).
  - Same 2 pre-existing `console.error` lines (`transformCallback`, `SettingsPanel`'s unguarded
    `listen()` in preview) — documented in the first pass, not from this story.
  - Artifact churn: only `01-shortcuts-paste-behavior.png` differs (render nondeterminism); the
    JSON evidence is byte-identical.
- **Surface-smoke-checklist traps, re-run mechanically for THIS round's diff:**

| Trap | Check | Result |
|---|---|---|
| #1 camelCase keys under `advanced` | derived every `AdvancedSettings` serde name from the Rust struct (container `rename_all = "camelCase"`, so the names are derived, not read from per-field attributes) and diffed against `src/types.ts` | **17 = 17**, neither side has a key the other lacks (this round adds/removes no key) |
| #2 resync `useEffect` / `isDirty` | both surviving advanced fields still in the mount load, the `isDirty` terms, the save gate and the merge; `...advancedSettings` still appears **nowhere**; `persistedAdv` 4 sites, `advancedSaveSkipped` 3 sites | green |
| #6 multi-hop positional save chain | parsed each hop's parameter list on the working tree and on `HEAD` with the **same** counter | `saveSettings` 52 = 52 · `handleSaveSettings` 50 = 50 · `SettingsPanel.onSave` 50 = 50 — this round touches no hop signature, so nothing shifted |
...
**Inversion table — FIX ROUND 3.** Every row was **observed**, not predicted. Two were observed
as the **pre-fix RED state** (the fixture cases were added before the twins were fixed, so the
drift that had to fail was the shipped trim-first code itself); one was a real re-introduction
after the fix and was reverted with a **targeted reverse edit**, not `git checkout`. `git status`
is clean of inversions.

| # | Drift (symbol) | Test(s) that went RED | Discriminating? |
|---|---|---|---|
| R3-1 | **Rust/RES-2** `llm::effective_cleanup_model` trimming **before** filtering (pre-fix state) | `llm::tests::spec_twin_constants_cleanup_model_sanitize` — `raw "\u{1c} deepseek"`, `left: " deepseek"`, `right: "deepseek"` | yes — fails only on the new edge-adjacent `U+001C` case; both round-2 NEL-in-isolation cases stayed **GREEN**, which is exactly the blind spot round 3 named |
| R3-2 | **Kotlin/RES-2** `KlarvoApi.effectiveCleanupModel` back to `override.trim().filter { … }` **after** the fix (re-introduced deliberately, then reverse-edited) | `TwinConstantsVectorsTest.cleanupModelSanitizeMatchesFixture` (9 tests, 1 failed) — `ComparisonFailure: raw [\x85,  , d, e, e, p, s, e, e, k] … expected:<[]deepseek> but was:<[ ]deepseek>` | yes — fails on the leading-NEL edge case, the mirror image of R3-1: each runtime is caught by the case the *other* one passes |
| R3-3 | **Rust/R3-3** `effective_cleanup_model`'s filter back to `>= '\u{20}'` only (dropping the `!= '\u{85}'` clause) | `llm::tests::spec_twin_constants_cleanup_model_sanitize` — `raw "dee\u{85}pseek"`, `left: "dee\u{85}pseek"` | **partly — and this is the honest limit of R3-3.** The failure comes from the explicit `assert_eq`, which runs *first*; the widened structural guard never got to speak. It cannot, by construction: every `expected` in the table is NEL-free, so a surviving `U+0085` always trips the equality assertion before the invariant. Widening the guard makes it *agree* with the predicate it sits beside (that was the finding) and future-proofs a case whose expected value contains one; it does not add a reachable failure today. |

**Not inverted, stated as such:** **R3-4 / R3-5** — both live on the branch that runs only when
`getAdvancedSettings()` **rejects**, which needs a backend; the browser proxy serves mocks and has
none, so there is no harness that can reach them. Verified by reading plus the mechanical trap-#2
check. **R3-1 (fixture description)** and **R3-2 (the two `PINS:` clauses)** are claim-accuracy
text with no behavioural assertion to invert — though the *code* order they now describe is the
one R3-1/R3-2 above prove. **R3-7** — an assertion *message*, not an assertion; inverting it
...
**FIX ROUND 3 — resolutions (2026-09-12).** Rows written from `git diff`, anchored by symbol
(Epic-7 retro D2). 7 of the 8 findings; **R3-6 is record-only and was left untouched** for the
conductor's close-out pass, as directed. The three round-3 `[Defer]` rows and every earlier
deferred row were likewise not touched.

- ✅ **RES-2 (now actually closed):** both twins changed from *trim → filter* to
  **filter → trim → empty check**. `llm::effective_cleanup_model` collects the filtered chars
  first and trims the result; `KlarvoApi.effectiveCleanupModel` is
  `override.filter { … }.trim()`. This removes the dependency on either runtime's whitespace
  table: the previous order let Kotlin's `trim()` stop at a NEL and Rust's `trim()` stop at a
  `U+001C`, each leaving the *adjacent space* behind on one platform only. Three edge-adjacent
  cases now lock it (leading NEL+space, trailing space+NEL, leading `U+001C`+space), and they
  discriminate in **both** directions — R3-1 in the inversion table fails the Rust half, R3-2
  fails the Kotlin half, on different rows of the same table.
- ✅ **R3-1:** the fixture description's two false claims are gone. "the two `trim()`
  implementations disagree about it and nothing else" and "(U+001C..U+001F … so they are
  **unobservable**)" are replaced by the measured statement: the two `trim()`s disagree in
  *opposite* directions (`U+0085` vs `U+001C`..`U+001F`), and both classes are observable via the
  neighbouring whitespace each runtime does or does not consume. The `PINS:` clause now states
  the filter→trim→empty order, and the description says explicitly that the NEL-in-isolation
  cases are **blind** to the ordering — which is why the edge-adjacent ones were added.
- ✅ **R3-2:** both fixture readers' `PINS:` clauses now state the predicate their code actually
  runs — filter (incl. `U+0085`) → trim → empty — and name which cases force which half of the
  order.
- ✅ **R3-3:** both structural guards widened to reject `U+0085` as well
  (`c < '\u{20}' || c == '\u{85}'` / `it.code < 0x20 || it.code == 0x85`), so the advertised
  invariant matches the predicate beside it. Measured limit recorded in the inversion table: the
  explicit rows still fire first, so the guard adds agreement and future-proofing, not a new
  reachable failure.
- ✅ **R3-4:** the skip report moved out of `if (!opts?.silent)`. The API-key-removal path
  (`clearApiKey(...).then(() => saveCurrentSettings({ silent: true }))`) now surfaces a rejected
  re-read instead of dropping the `silenceThreshold`/`pasteDelayMs` edits silently. The log half
  was already unconditional (the `catch` at the failure site); the declaration comment now says
  so instead of promising only "the save banner below".
- ✅ **R3-5:** `setTimeout(() => setSaveMsg(null), 2000)` now applies **only** to the plain
  "Saved" toast. The danger-styled skip notice has no timer and stays until the next save
  replaces it — it is a state the user must act on, not a success toast.
- ✅ **R3-7:** the `else`-branch assertion message no longer states a per-label rationale that
  holds for one label only. It now reads: the slot holds DeepSeek off Windows, **and the
  predicate ORs all four overrides regardless of provider**, so any `llm_model_*` change (here
  `llm_model_{label}`) must still rebuild. Behaviour unchanged — this is the message, not the
  assertion.

**Still open after FIX ROUND 3** (unchanged by this round, repeated so the close-out does not
have to reconstruct them):

- **Andi's GATE-4 is still outstanding** — Windows release build via `scripts/windows-build.sh`;
  Advanced panel shows only live keys; a changed DeepSeek model ID visible in `Klarvo.log`; plus
  the two items round 1 added (the D2 warning text on a typo'd ID, no Trial badge on the "Text
  Cleanup" row). Nothing in this session ran on Windows or in the Tauri runtime.
- **The "stale advanced snapshot" round trip is still not driven end-to-end** — fixed in code
  (P1 + REG-3 + R3-4/R3-5), but the browser proxy has no backend. The concrete GATE-4
  observation that closes it is unchanged: set a DeepSeek model ID in Advanced, change the
  silence threshold in Settings and save, reopen Advanced — the model ID must still be there.
- **`LocalLlmCleanup::model()` (P3) and REG-1's Windows branch are still not compiled by any
  gate that ran** — both are `#[cfg(target_os = "windows")]` / `cfg!(windows)`; the Linux gate
  compiles the other half only.
- **No device/emulator Android smoke this round either** — the JVM half was run device-free (see
  the Debug Log for how the generated tree was brought in sync without the usual copy step).
...
**FIX ROUND 3 — files touched (2026-09-12), by finding**

*Rust (desktop)*
- `src-tauri/src/llm/mod.rs` — RES-2: `effective_cleanup_model` now **filters first, then
  trims** (`filter(..).collect::<String>().trim()`), so neither runtime's whitespace table can
  leave a neighbouring space behind (+ doc rewritten to state the filter → trim → empty-check
  order and why each step sits where it does); R3-2: the `PINS:` clause of
  `spec_twin_constants_cleanup_model_sanitize` states the new predicate incl. `U+0085` and names
  the edge-adjacent cases that force the order; R3-3: its structural guard widened to
  `c < '\u{20}' || c == '\u{85}'`
- `src-tauri/src/commands/settings.rs` — R3-7: the `else`-branch assertion message in the
  `cleanup_provider_reload_needed` loop no longer claims the slot rationale holds per label; it
  now says the predicate ORs all four overrides regardless of provider

*Kotlin (Android)*
- `android/kotlin-src/com/klarvo/voice/KlarvoApi.kt` — RES-2: `effectiveCleanupModel` is
  `override.filter { … }.trim()` (the twin of the Rust change), + KDoc rewritten the same way
- `android/kotlin-test/com/klarvo/voice/TwinConstantsVectorsTest.kt` — R3-2: `PINS:` clause of
  `cleanupModelSanitizeMatchesFixture` updated to the new predicate; R3-3: guard widened to
  `it.code < 0x20 || it.code == 0x85`

*Frontend (TS/React)*
- `src/components/SettingsPanel.tsx` — R3-4: the `advancedSaveSkipped` report moved **outside**
  `if (!opts?.silent)`, so the API-key-removal path (the one silent caller) can no longer swallow
  it; R3-5: the 2000 ms auto-dismiss now applies **only** to the plain "Saved" toast — the skip
  banner stays until the next save replaces it; the flag's declaration comment corrected to say
  it is reported twice, both times regardless of `silent`

*Fixtures / evidence / tracking*
- `test-fixtures/twin-constants-vectors.json` — RES-2: **3 edge-adjacent cases** added to
  `TWIN-CLEANUP-MODEL-SANITIZE-001` (9 → 12 cases; entry count unchanged at 10): leading
  `U+0085`+space, trailing space+`U+0085`, leading `U+001C`+space; R3-1: the description's two
  measurably false claims removed ("disagree about it **and nothing else**"; "U+001C..U+001F …
  **unobservable**") and replaced with the measured, symmetric statement plus the corrected
  filter→trim order
- `_bmad-output/implementation-artifacts/gate4-evidence/7-9/01-shortcuts-paste-behavior.png` —
  re-run artifact (render nondeterminism; the JSON evidence is byte-identical, harness unchanged)
- `_bmad-output/implementation-artifacts/7-9-desktop-advanced-settings-dead-keys-and-model-ids.md`
  — this record; the 7 round-3 finding checkboxes ticked (R3-6 left for the close-out pass)
...
| 2026-09-12 | Addressed code review findings — fix round 3 (final, human-authorized beyond the fix cap): 7 items resolved (RES-2, R3-1..R3-5, R3-7); R3-6 (record-only) and all deferred rows untouched. |
...
| 2026-09-12 | **Re-opened after Andi's GATE-4** (points 1/2/4 green, point 3 red): the degrade warning is unreadable on Windows — `native_pill.rs` lets the follow-up Done overwrite a Warning within ms (root: 12-1 native re-port assumption), and DeepSeek's live 400 wording (`The supported API model names are …, but you passed X.`) matches no `is_model_not_found_error` needle because `llm/mod.rs` keeps only `error.message` (the 7-9 fixture fed the whole JSON body). Causes isolated in `gate4-evidence/7-9/verdict.md` §"Andi's GATE-4"; fix pending the display decision (human gate). |
| 2026-09-12 | Fix 3a+3b (`06603d2`): the native pill holds a Warning 4 s and ignores the plain Done inside the hold (`warning_hold_active`); `is_model_not_found_error` gained DeepSeek's live needles, spec test carries the verbatim log message. Windows build green; Andi's re-check of point 3 pending (verdict.md §"Fix round"). |
| 2026-09-13 | Andi's re-check of point 3 green on `06603d2` (warning readable, hot-reload works) → story done, epic-7 done. |
...
- [x] [Review][Patch] REG-1's new else-branch assertion message states a rationale true for one of its four labels — "the slot holds DeepSeek and a changed `llm_model_{label}` must still rebuild" runs for deepseek/openai/groq/anthropic, but only `llm_model_deepseek` affects that slot; the other three pass because the predicate ORs all four regardless of provider [src-tauri/src/commands/settings.rs::tests (the `cleanup_provider_reload_needed` loop)]
...
### Review Findings — round 4 (re-review of fix round 3; human-authorized beyond the fix cap)

Re-review (bmad-code-review, 2026-09-12) over `f164056..HEAD`, judging commit `9015a0a` against the
eight round-3 findings. Three layers ran, none failed: Blind Hunter (diff only), Edge Case Hunter
(diff + tree), Acceptance Auditor (diff + spec + context). Scope was verification of those findings
and of the lines `9015a0a` touched — **not** a fresh adversarial sweep. Anchors are `file::symbol`.
The twin question was settled by **running both runtimes** (real `rustc`, real JDK 17 emulating
Kotlin's `Char.isWhitespace() = Character.isWhitespace || Character.isSpaceChar`), not by reasoning.

**Round-3 verdicts — all seven code findings resolved; R3-6 untouched as directed:**
- **RES-2 — resolved, on the merits.** Both twins are now filter → trim → empty check
  (`llm::effective_cleanup_model` collects the filtered chars then trims the result;
  `KlarvoApi.effectiveCleanupModel` is `override.filter { … }.trim()`). Verified **exhaustively**,
  not by the table: over every scalar value, Rust's post-filter `is_whitespace` set and the JVM's
  post-filter `isWhitespace ∪ isSpaceChar` set are **byte-identical** —
  `0020,00A0,1680,2000..200A,2028,2029,202F,205F,3000`. The two divergent classes are exactly
  `{U+0085}` (Rust-only) and `{U+001C..U+001F}` (Kotlin-only), and the filter removes both before
  either `trim()` runs. So the whole divergence class is closed, not just the three new rows. All
  eleven cases produce identical output on both runtimes. The new rows discriminate asymmetrically
  as claimed: trim-first Kotlin fails `"\u{85} deepseek"` → `" deepseek"` and `"deepseek \u{85}"` →
  `"deepseek "`, trim-first Rust fails `"\u{1c} deepseek"` → `" deepseek"` — each runtime caught by
  the row the other passes.
- **R3-1 — resolved.** Both false claims are gone from the fixture description and the stated order
  is filter → trim → empty. Its measured statements re-verified true: Rust `str::trim` strips
  `U+0085` and keeps `U+001C..U+001F`; the JVM predicate is the exact opposite
  (`isWhitespace(U+001C..U+001F) = true`, `U+0085` false on both halves); `U+00A0` does not diverge
  (JVM catches it via `isSpaceChar`). One clause it carried over is now false — see P1.
- **R3-2 — resolved.** Both readers' `PINS:` clauses state the predicate their code runs. Two
  defects inside the new text — see P1 and P2.
- **R3-3 — resolved.** Both structural guards reject `U+0085`
  (`c < '\u{20}' || c == '\u{85}'` / `it.code < 0x20 || it.code == 0x85`). The story's own inversion
  table already records the honest limit (the explicit `assert_eq` fires first, so the widened guard
  adds agreement and future-proofing, not a reachable failure) — confirmed, and therefore not raised.
- **R3-4 — resolved.** `SettingsPanel::saveCurrentSettings` reports the skip outside the silent gate
  (`if (advancedSaveSkipped) … else if (!opts?.silent) …`), so the API-key-removal path can no longer
  swallow it. The declaration comment's "logged where it happens" is accurate — the `console.error`
  sits in the inner `catch` at the failure site and is unconditional.
- **R3-5 — resolved as written, but its effect is defeated.** `setTimeout(… , 2000)` now sits only
  in the plain-"Saved" branch. The banner can still vanish after two seconds — see P3.
- **R3-7 — resolved and accurate.** The else-branch message now reads "the predicate ORs all four
  overrides regardless of provider", which matches `cleanup_provider_reload_needed`: the only early
  return is `cfg!(target_os = "windows") && llm_provider == "local"`, and the body ORs all four
  `llm_model_*` comparisons.
- **R3-6 — untouched, as directed** (record-only, conductor close-out).

**Gates re-run independently this session (not taken from the record):** `cargo test --lib`
**681 passed / 0 failed / 0 ignored** · `npx tsc --noEmit` **clean** · JVM
`./gradlew :app:testUniversalDebugUnitTest --rerun-tasks` **186 tests / 0 failures / 0 errors /
0 skipped across 23 suites**, counted over all 23 result XMLs, `cleanupModelSanitizeMatchesFixture`
present with 0 failures. The generated Kotlin tree is byte-identical to `android/kotlin-src` and
`android/kotlin-test` (`diff -rq`, only the Tauri-generated `generated/` dir extra), so the JVM gate
ran against the committed sources. `git status` clean before and after. This closes the round-3
record's two open measurement gaps (RES-2's Kotlin half and the JVM re-measurement).

- [ ] [Review][Patch] **The lone-`U+0001` rationale is false under the order the same sentence now states — in all five copies** — measured: `U+0001` is removed *by the filter*, so it never reaches `trim()`. Under filter-first it discriminates nothing (filter → empty-check → trim also yields the default). The vector that actually forces "empty check last" is the whitespace-only `"  "` row, already in the table (measured: filter → empty-check → trim returns `""`, not the provider default → RED). Both `PINS:` clauses claim "it survives `trim()` and would be returned verbatim by a strip-after-the-empty-check implementation" and the fixture description calls it "non-empty after filtering but blank after trimming" — the first is impossible and the second is the opposite of what happens. Same claim-accuracy class R3-1/R3-2 were raised to kill, reintroduced by their own rewrite. Fix direction: name `"  "` as the forcing vector and drop the `U+0001` rationale (or keep `U+0001` only as a "filter runs at all" vector) [src-tauri/src/llm/mod.rs::effective_cleanup_model (docstring), src-tauri/src/llm/mod.rs::spec_twin_constants_cleanup_model_sanitize (PINS), android/kotlin-src/com/klarvo/voice/KlarvoApi.kt::effectiveCleanupModel (KDoc), android/kotlin-test/com/klarvo/voice/TwinConstantsVectorsTest.kt::cleanupModelSanitizeMatchesFixture (PINS), test-fixtures/twin-constants-vectors.json::TWIN-CLEANUP-MODEL-SANITIZE-001 description clause (1)]
- [ ] [Review][Patch] **The Kotlin `PINS:` clause pairs the divergent characters with the wrong runtimes, and now contradicts its Rust twin** — it reads "Kotlin and Rust disagree in opposite directions (U+0085 vs U+001C..U+001F)", which positionally assigns `U+0085` to Kotlin. Measured the other way round: the JVM predicate is **false** for `U+0085` and **true** for `U+001C..U+001F`; Rust is the mirror. The Rust twin's clause has the pairing right ("Rust and Kotlin … (`U+0085` vs `U+001C`..`U+001F`)"), so the two halves of a deliberately twinned docstring now state opposite facts — the divergence R3-2 exists to prevent. Fix direction: swap the subjects in the Kotlin clause, or name each runtime with its own character class explicitly [android/kotlin-test/com/klarvo/voice/TwinConstantsVectorsTest.kt::cleanupModelSanitizeMatchesFixture (PINS)]
- [ ] [Review][Patch] **R3-5's effect is defeated: the untracked 2 s timer still clears the skip banner** — the `setTimeout(() => setSaveMsg(null), 2000)` armed by a plain "Saved" is never stored and never cancelled, so it fires over whatever `saveMsg` holds two seconds later. Reachable on exactly the path R3-4 was raised for: a non-silent save succeeds (timer armed) → within 2 s the user removes an API key → `clearApiKey(...).then(() => saveCurrentSettings({ silent: true }))` skips the advanced block → `setSaveMsg("Saved — advanced settings skipped …")` → the pending timer fires → the danger banner disappears. That is the "self-dismisses after two seconds" state R3-5 was written to remove. The same file already has the correct pattern (`apiKeyConfirmTimers` ref + `clearTimeout` + unmount cleanup). Not caught by any gate: the proxy smoke cannot reach the skip path, as the record states. Fix direction: hold the timeout id in a `useRef` and `clearTimeout` it before every `setSaveMsg` [src/components/SettingsPanel.tsx::saveCurrentSettings]
- [ ] [Review][Patch] **A successful *silent* save neither clears nor replaces a stale skip banner, so R3-5's "stays until the next save replaces it" is false for the one silent caller** — the reset at the top of the function is gated (`if (!opts?.silent) setSaveMsg(null)`), and a silent save with `advancedSaveSkipped === false` matches neither the `if` nor the `else if`. A skip banner therefore survives a later *successful* API-key-removal save untouched, still telling the user the advanced block was skipped. Claimed in both the new in-code comment and the round-3 resolution row. Fix direction: `else { setSaveMsg(null); }` on the silent-success path, or narrow the claim to non-silent saves [src/components/SettingsPanel.tsx::saveCurrentSettings]

- [x] [Review][Defer] The Completion Notes still state the superseded predicate as current — the AC5 bullet reads "`llm::effective_cleanup_model` (trim, then empty → the provider's `DEFAULT_MODEL`)" and the D2 bullet reads "now **trim, then drop every char < U+0020, then** apply empty → default — the order matters and the fixture forces it". Both describe today's code and both are now false; `9015a0a` fixed the fixture and the readers but not the record [this file, Completion Notes List / AC5 + D2 bullets] — deferred, record-only, same class as R3-6 → conductor close-out
- [x] [Review][Defer] The round-2 RES-2 resolution row still carries the claim R3-1 was raised to delete — "(… `U+001C`..`U+001F` diverge the other way but are **unobservable because both filters already drop them**)" and, in the present tense about the trim-first code round 3 refuted, "the result **no longer depends on either runtime's whitespace table**" [this file, "FIX ROUND 2 — resolutions", RES-2 row] — deferred, record-only → conductor close-out
- [x] [Review][Defer] "So neither runtime's whitespace table matters" is an invariance overclaim in both production docstrings — both `trim()`s still run over everything ≥ `U+0020`; the claim holds because the two tables *happen to* coincide above `U+0020` at the shipped versions. Verified true today by exhaustive comparison (identical sets), but it is not established by the code and is exposed to Unicode-table skew between two independently updated stdlibs, and no vector covers any ≥ `U+0020` whitespace (`U+00A0`, `U+202F`, `U+2007`) [src-tauri/src/llm/mod.rs::effective_cleanup_model, android/kotlin-src/com/klarvo/voice/KlarvoApi.kt::effectiveCleanupModel] — deferred, residual: true as measured, weak as an invariant; not fixed this round per the review scope
- [x] [Review][Defer] No assertion pins the *presence* of the three ordering vectors — deleting them leaves both readers green, because the completeness tests count fixture **entries**, not cases (`fixtureCarriesAllTenEntriesAndDescribesEach` / `spec_twin_constants_fixture_is_complete_and_self_describing`). The ordering lock RES-2 rests on is therefore removable without a failure. Fix shape: assert the three `raw` values exist, the way the entry count is asserted [test-fixtures/twin-constants-vectors.json::TWIN-CLEANUP-MODEL-SANITIZE-001, src-tauri/src/llm/mod.rs::spec_twin_constants_cleanup_model_sanitize, android/kotlin-test/com/klarvo/voice/TwinConstantsVectorsTest.kt::cleanupModelSanitizeMatchesFixture] — deferred, residual, not fixed this round per the review scope
- [x] [Review][Defer] R3-4 created an asymmetry it did not close: a *skip* is now reported regardless of `opts.silent`, but an outright **rejection** on the same silent path is still fully swallowed — the outer handler is `catch (err) { if (!opts?.silent) setSaveMsg(…) }` with no log. So the API-key-removal save can fail entirely with no banner and no console trace, while the lesser failure (skip) is now surfaced [src/components/SettingsPanel.tsx::saveCurrentSettings (outer catch)] — deferred, residual: the line itself is untouched by round 3, the inconsistency is new

**Dismissed as noise (18, with the check that killed them):** "the JVM 186/186 green predates the deliberate Kotlin inversion and no post-revert green exists" (the only surviving artifact *was* RED — `tests=9 failures=1`, 16:14, the recorded R3-2 inversion, against a 16:19 commit — so the inference was fair, but the gate was **re-run green this session**: 186/186 across 23 suites against a generated tree verified byte-identical to the committed sources) · "the skip banner may render as success if the styling uses `startsWith`" (the render site uses strict equality, `saveMsg === "Saved"` / `!== "Saved"`) · "'the one silent caller' will rot" (`grep "silent: true" src/` → exactly one site; speculative future drift) · "the new comment points at the wrong `catch`" (the inner `catch` carrying the unconditional `console.error` *is* below the comment) · "R3-3's widened guard is unfalsifiable" (true, and the story's own inversion table records exactly that limit; R3-3 asked for agreement with the adjacent predicate, which is what landed) · "'observed RED on both halves' is asserted without evidence" (the record quotes both failures verbatim, and both were reproduced independently here) · "`U+007F`, the other C1 controls, `U+2028`/`U+2029` survive the filter, and there is no length cap" (identical on both twins, so no divergence, and D2 explicitly chose no allowlist and no cap — the fixture description carries that disclaimer) · "R3-7 broadened rather than narrowed the message, and states the implementation inside its own failure text" (it is longer and strictly more accurate, which is what R3-7 asked for; every clause checks out against `cleanup_provider_reload_needed`) · "the reorder adds a `to_string()` allocation" (one short string per provider construction, not per token) · "`persistedAdv === null` misses a resolved `undefined`" (already dismissed in round 3 — the command resolves the struct or rejects) · "a mount-time `advancedSettings === null` reports Saved while nothing persisted" (round-1 deferred row, verbatim) · "overlapping saves are not gated on `saving`" (pre-existing, untouched) · "an unpaired surrogate diverges between the twins" (narrow, at the config parse boundary; the round-1 deferred non-string row's family) · "the commit body mis-states which code the dropped `unobservable` claim was false about" (it was false of the trim-first code the commit removes, which is what the round-3 finding said; the body is compressed, not wrong) · "two of the three new rows are redundant and there is no trailing `U+001C` counterpart" (Kotlin is failed by two rows and Rust by one; the record states that asymmetry, and the leading row already fails Rust) · "the shared fixture would hide a loader bug" (each half feeds its **own** production seam; the seams are what the lock discriminates) · "only the `else` arm compiles on this host, so R3-7's contrast is untested" (the record states exactly that limit) · "four hand-maintained copies of the control-character set with no shared constant" (they are cross-language twins — ADR-0017 — and the fixture *is* the shared lock; a Rust constant cannot reach Kotlin).

**Coverage of this re-review (what it did NOT do):** no fresh adversarial sweep of the story — only the eight round-3 items and the lines `9015a0a` touched. The three gates re-run here cover Rust lib tests on **Linux** (the Windows arms — `llm::local`, the `local` cleanup arm, REG-1's `cfg!` branch — are still not compiled), strict `tsc` (which executes nothing), and pure Kotlin/JVM logic (no JNI, no device, no emulator, no `readConfig` file I/O). Not covered by anything here: the Tauri runtime, `save_advanced_settings` as a command, any network call, the real `Klarvo.log`, `tests/pi_security.rs`, the desktop proxy smoke (not re-run this session — `src/components/SettingsPanel.tsx` was **not** changed by this review), and **all four patch findings above**: P1/P2 are claim-accuracy text with no behavioural assertion, and P3/P4 live on the skip branch that needs a rejecting backend, which no harness in the repo can reach. The Unicode-table comparison behind RES-2's verdict holds for **this host's** `rustc` and JDK 17; a different JDK's Unicode version was not tested.
