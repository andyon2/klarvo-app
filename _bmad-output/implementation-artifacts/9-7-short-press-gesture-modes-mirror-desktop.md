...
- 2026-06-16 (resolution) → **DONE.** Built an instrumented (logging-only) APK and Andi reran on the real Xiaomi: Auto mode **works** (~90%). The earlier "totally broken" was a **stale build** on the phone (current 9-7 code had never been installed). Device VAD telemetry proves the modes are correct — `tap=auto→AUTO onSilenceDetected=true`, `speech onset confirmed` → `silence detected (62 frames)` → flush → loop restart. 9-7's real scope (4 modes available + Auto loop behaves) is MET and log-backed. The residual ~10% miss is a **separate concern, NOT a mode-mapping bug**: Android ignores the user's `silence_threshold` setting and hard-codes the VAD energy pre-gate at `0.02` (4× the desktop default `0.005`), so quiet speech (device mic peaks ~0.026–0.037) fails the 3-consecutive-frame onset and merges into the next louder utterance. Routed to **new story 9-11** (Android honors `silence_threshold` + default 0.005 + multi-fire guard) + a follow-up "sensitivity hint" story. Andi's call (2026-06-16): close 9-7, build 9-11 now.
