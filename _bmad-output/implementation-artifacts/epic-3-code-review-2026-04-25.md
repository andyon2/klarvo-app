...
- [ ] **[Review][Patch] P16 — Spec-Text-Fix: `SystemClock` → `MonotonicClock`** [_bmad-output/planning-artifacts/epics/epic-3/story-3.10*.md AC-C Step 7, story-3.11*.md AC-B] — `SystemClock` existiert nicht in klarvo-core; Code verwendet korrekt `MonotonicClock` (ADR-0001/0003 + memory). Fix: Spec-Text in beiden Stories nachziehen.
