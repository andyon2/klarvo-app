...
**Test deps:** insta 1 (snapshots) · wiremock 0.6 (HTTP doubles) · puppeteer ^24.38.0 (desktop machine gate — a `package.json` dependency, drives real Chromium against `npm run preview`)
...
**ADRs in force:** 0015 state-file writes · 0016 Android path parity · 0017 shared-core STT path · 0018 Android bubble rendering tech · 0019 cross-platform design SSOT · 0021 native desktop overlays. **ADR-0020 (WebView2 fixed-runtime pin) is superseded by 0021** — it stays as a measurement record only; do not re-apply the runtime pin.
...
- **Tests are inline `#[cfg(test)]` modules by default.** Snapshot tests use `insta`; snapshots live in `src-tauri/src/snapshots/`. Accept with `cargo insta review`. **The one integration suite is `src-tauri/tests/pi_security.rs`** (prompt-injection, Arcanum taxonomy): only `cargo test --test pi_security output` runs key-free; the tier/baseline tests are `--ignored` and need `GROQ_API_KEY` (or `PI_PROVIDER=deepseek` + `DEEPSEEK_API_KEY`). Don't add a second `tests/` file for unit-level checks — keep those inline.
- **Linux `cargo test` + lint do NOT satisfy the DoD for surface/UI stories.** Hard gate: a real **Windows release build via `scripts/windows-build.sh` + manual press-to-paste smoke** is required. (`cargo check` and Linux tests mask Tauri-runtime bugs and Windows-only code paths.) **Before the smoke, run the applicable items of `docs/surface-smoke-checklist.md`** — the running ledger of traps that are green on Linux (camelCase config keys, Settings resync-`useEffect`, FloatingBar separate-window reactivity, window-geometry/region clip, event push-wiring). Mechanical check, not a self-attestation (Epic-5 retro AI-1).
...
- **Never mutate the host to reach a gate.** No `apt`, `rustup target add`, `pip`, `cargo install`, `npm install -g` from a story. If the tool for a gate is missing, the gate is `blocked` — report it, don't install around it. A gate that stays red across two consecutive stories is **retired**, not chased. **`cargo check --target x86_64-pc-windows-gnu` is such a retired gate** (dead at `ort-sys` since Story 8-1; two Epic-8 workers installed mingw on the host for it anyway). Do not write it into any DoD or task list; the only Windows compile statement comes from `scripts/windows-build.sh`.
...
- **Canonical Windows build is `scripts/windows-build.sh [<ref>] [--clean] [--skip-npm]`** — run it from powerhouse, which has no Windows. It (1) refuses a dirty tree, (2) **pushes the branch to `origin`**, (3) checks it out on the laptop over SSH, (4) runs `sync-and-build.ps1` there (robocopy from the laptop's WSL checkout → `D:\apps\klarvo` → Tauri build), (5) proves freshness — the exe must be younger than the built commit. Exit codes: 0 fresh · 1 precondition (dirty tree, laptop unreachable, SHA drift) · 2 the Windows build itself failed (compiler errors in the log) · 3 stale exe. **Run it in the background** — it takes minutes to ~30 min. `sync-and-build.ps1` is the inner laptop step; do not call it directly from powerhouse. In a non-interactive SSH session `powershell.exe` is not on PATH — the script uses the absolute `System32` path; do the same if you ever bypass it. **Signing gotcha:** the Tauri signer hangs → run `rsign` afterwards (`scripts/sign-installer.sh`).
- **Every build pushes.** Because step (2) is unconditional, "commits are pushed only at releases" is no longer true for this repo. Unpushed commits exist only until the next build; do not rely on the laptop seeing a state that hasn't gone through `origin`.
...
- **Build freshness differs per platform.** Desktop: Settings → About shows `Build <hash> · <timestamp>`, served by the `get_build_info` command (and `windows-build.sh` exit 3 catches a stale exe). Android: there is **no in-UI version screen** — verify a fresh APK landed via `scripts/android-build.sh`'s timestamp gate + APK filename (or `adb … lastUpdateTime`), NOT a version number.
- **Unattended multi-story runs (epic-/story-conductor) read their orchestration contract from `_bmad/custom/bmad-epic-conductor.toml`** — run-guard, smoke surface, `BMAD_CONDUCTOR` flag, worker grant, and visual-oracle posture live there (generator-immune, conductor-only), NOT in this file. Do not re-add conductor mechanics here.
...
- **A number states what it covers.** "18/18 green" is not a result until it names what was *not* exercised. 8-8 reported 18/18 while four of five Copy sites were never clicked; the review had to add that. Every gate verdict, test count and smoke report says explicitly which states, sites or platforms it did not touch — a finding without its coverage statement is not a finding.
