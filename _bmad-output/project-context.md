...
- **Android smoke runs unattended on the emulator; the real device is only the human's visual gate.** Build/install/drive-to-state via `scripts/android-smoke.sh`. The **unattended** surface is a headless emulator (`scripts/android-emulator.sh`; install `--abi arm64-v8a` so the arm64-only Rust `.so` runs via the x86_64 image's native-bridge). The **real Xiaomi/HyperOS device is NOT unattended-producible** — its install confirmation needs a finger, and the `DEBUG_SET_STATE` harness is dead on HyperOS (background restrictions) — so it is Andi's *batched* gate, never an automated-run target. `android-smoke.sh` also runs a build-time gate (`node scripts/gen-android-theme.mjs --check`) that fails if `KlarvoTheme.kt` drifted from the canon CSS (ADR-0019) — regenerate, don't hand-edit.
...
- **Unattended multi-story runs (epic-/story-conductor) isolate via `scripts/conductor-guard.sh`** (run-lock + HEAD-watcher + it *detaches the real device* for the run's duration) and export `BMAD_CONDUCTOR=1` (forces `android-smoke.sh` to the emulator and hard-rejects any non-emulator target). This is why an unattended run physically cannot install onto the real phone.
...
- **The Android emulator is a structural oracle, not a pixel oracle.** It reliably confirms compile / logic / install / drive-to-state AND — via `adb shell dumpsys window windows` — the **overlay-window structure** (how many `APPLICATION_OVERLAY` windows, their requested sizes / gravity / visibility). That layer is machine-checkable and unattended-trustworthy: e.g. the recording state must hold exactly the panel (~1080×525, bottom) + the idle bubble (162×162); a regression to a wrong window count/size is caught here without a screenshot. What it does **NOT** reproduce faithfully is the **pixel / aesthetic layer** — stock Android (no HyperOS skin), software GPU, and system dialogs can force-hide overlays (`mForceHideNonSystemOverlayWindow`); on 2026-06-15 an emulator screenshot looked "green" while the real Xiaomi was visibly broken. **Assert overlay structure on the emulator (unattended gate); the visual/aesthetic verdict is Andi's real-device gate, never an emulator screenshot.**
