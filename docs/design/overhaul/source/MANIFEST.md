...
- **sourceFingerprint:** `975cc536ca9c1b8476f1509bb1a1f9a8` (md5 over `Klarvo Design System.html` + `assets/klarvo.css`)
...
| 2026-09-14 | `01d4d0e35d7fa302f0c5bba87e2048b3` | **Pill: Cleanup-Degrade-Zustand** (Story 7.10, GATE-1-Entscheidungen von Andi). Scheitert das Cleanup, wird der Rohtext NICHT mehr eingefügt und nie auto-gesendet — nur Zwischenablage. Die Pill erhält dafür **keinen neuen Zustand**: EIN terminales Ereignis (`DoneClipboard` trägt den Warntext) statt Warning→DoneClipboard, damit kein Folge-Ereignis die Model-ID löscht (7-9 Finding 3a). Amber, bestehende 4 s, Fläche 200×36 bleibt. Wortlaut englisch wie alle Pill-Labels: „Cleanup failed — raw text in clipboard (Ctrl+V)" bzw. „Model '<id>' not found — in clipboard" (Ursache vorn, überlebt das Abkürzen). Android-Toast spiegelt den Wortlaut als EIN kombinierter Toast, aber ohne „(Ctrl+V)" (GATE 2, Andi: kein Ctrl+V am Handy). Offen (Backlog, nicht hier): die Canon-Zeile „In Zwischenablage" ist deutsch, der Code rendert „In Clipboard". | [Story 7-10](../../../_bmad-output/implementation-artifacts/7-10-cleanup-failure-raw-text-clipboard-only.md) GATE-1 · Quelle `docs/backlog.md` DECIDED 2026-09-13. |
| 2026-09-14 | `7fac3dab5c665183a5533d48bac5b619` | **Pill ist Statuslicht, die Preview-Karte trägt die Nachricht** (Story 7.10, GATE-4-Runde 1: Andis Windows-Test zeigte den Warntext auf der 200×36-Pill mitten im Satz abgeschnitten, der Zwischenablage-Hinweis fehlte). **Ersetzt die Zeile 2026-09-14 darüber** (dort „hinten abgekürzt"): kein Text mehr auf der Pill für Nachrichten. Pill: statische Labels je Zustand (`Cleanup failed` amber / `In Clipboard` amber / `Warning` / `Error`). Nachricht: die vorhandene native Preview-Karte (8 px über der Pill, Amber- bzw. Danger-Linie) mit Kopfzeile, Ursache, Ort des Textes + Handlung, Hinweis; 4 s + 1 s Fade; neues Diktat oder Klick schließt; ersetzt bei aktiver Live-Preview den Vorschautext. Gilt für ALLE Pipeline-Nachrichten (Cleanup-Degrade, STT-Ladder, Boot-Warnung, Fehler) — eine Nachrichtenfläche, die Pill-Textkürzung entfällt. Sprache englisch. Andi-approved via `docs/design/overhaul/mockup-7-10-message-card.html` (Empfehlungen 1–4 übernommen). | [Story 7-10](../../../_bmad-output/implementation-artifacts/7-10-cleanup-failure-raw-text-clipboard-only.md) GATE-4 re-open 2026-09-14. |
| 2026-09-14 | `3d8c9b808854c27b43f7e1597700911d` | **Nachrichtenkarte: Nachträge aus dem AC8-Review** (Story 7.10, Andi D1–D3 + DN1). **D1:** passt eine Karte (Kartenhöhe, nicht die Fenster-Obergrenze) oben nicht, kippt die Karte 8 px UNTER die Pill; die Live-Preview folgt derselben Regel. **D2:** Cleanup-Fehler + gescheiterter Zwischenablage-Schreibvorgang = Danger-Linie, Kopf „TEXT LOST", „Clipboard write failed — raw text is in History", kein Ctrl+V-Hinweis; Pill-Label bleibt „Cleanup failed". **D3:** bekannte Maschinen-Tokens (z. B. `feature_requires_license:CommandMode`) werden auf Nutzertext übersetzt, unbekannte bleiben wörtlich. Boot-Warnungen: mehrere → EINE Karte. | [Story 7-10](../../../_bmad-output/implementation-artifacts/7-10-cleanup-failure-raw-text-clipboard-only.md) AC8-Review 2026-09-14. |
| 2026-09-14 | `975cc536ca9c1b8476f1509bb1a1f9a8` | **Nachrichtenkarte hält 8 s statt 4 s** (Story 7.10, GATE-4-Runde 2 grün; Andi: „blendet mir zu schnell ab, verlängern um ca. 3–4 s“). Karte 8 s + 1 s Fade; Pill bleibt bei 4 s (Statuslicht, nicht der Lesetext). Nur die Dauer, sonst nichts. | [Story 7-10](../../../_bmad-output/implementation-artifacts/7-10-cleanup-failure-raw-text-clipboard-only.md) GATE-4 Runde 2, 2026-09-14. |
