...
- **2026-03-22:** rust-core Agent hat beim Rename user-facing Strings in `#[cfg(target_os = "windows")]` Code geaendert, dabei Imports (MenuItem, Menu, TrayIconEvent) und Variablen-Prefixe (_saved_bar_x) kaputt gemacht. `cargo test` in WSL war gruen weil Desktop-only Code dort nicht kompiliert wird. Erst der Windows-Build deckte 7 Fehler auf. Merke: Nach Agent-Aenderungen an Rust-Code IMMER pruefen ob `#[cfg(desktop)]` oder `#[cfg(target_os)]` Bloecke betroffen sind. Diese werden von `cargo test` auf Linux NICHT geprueft. Bei Rename-Tasks explizit instruieren: "Aendere NUR den String-Inhalt, nicht Variablennamen oder Imports."
- **2026-03-22:** Bulk-sed fuer Tailwind-Klassen: `text-zinc-50` matched innerhalb von `text-zinc-500` → ergab `text-voxlit-text0` (kaputte Klasse). Merke: Bei sed-Ersetzungen laengere Patterns ZUERST verarbeiten (500 vor 50), oder Wort-Grenzen nutzen.
- **2026-03-24:** Tauri invoke Parameter-Name-Mismatch: Frontend sendete `{ state }` aber Rust-Command erwartete `onboarding_state` (→ camelCase `onboardingState`). Stiller Fehler — kein Crash, kein Error im UI, Onboarding wurde nie als "completed" gespeichert. Merke: Bei jedem `invoke()`-Call die Keys gegen die Rust-Funktionssignaturen pruefen. Tauri mappt `snake_case` Parameter auf `camelCase` Keys.
- **2026-03-24:** Audio-Pfade divergieren: Haupt-Recording-Pipeline (`encode_to_wav_with_gain`) downmixt Stereo→Mono korrekt. Der VAD-Pfad (fuer Auto-Stop Silence Detection) tat das nicht — sendete interleaved Stereo an Silero, das 16kHz Mono erwartet. Ergebnis: Auto-Stop funktionierte nie auf Stereo-Mics (Laptops). Merke: Bei parallelen Datenpfaden zum selben Audio immer BEIDE auf korrekte Kanalbehandlung pruefen.
- **2026-03-24:** UI Section-Header-Farben: Nicht statisch Free=Teal / Paid=Muted zuweisen. Dynamisch per `isPaid`: Free-Sektionen immer Teal, Paid-Sektionen Teal wenn lizenziert, Muted+Lock wenn unlizenziert. Dem zahlenden Kunden keine "das ist eigentlich paid"-Signale zeigen.
