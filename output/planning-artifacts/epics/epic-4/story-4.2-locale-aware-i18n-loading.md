...
- [x] Task 1 — `load(ui_language)` implementieren (AC-A, AC-E)
  - [x] 1.1 `load_default()` durch `pub fn load(ui_language: &str) -> Arc<I18nTable>` ersetzen
  - [x] 1.2 Eager-Validation beider Locale-Files (both panic-on-invalid-JSON)
  - [x] 1.3 Match-Arm: `"de"` → de-table, `_` → en-table
  - [x] 1.4 Phase-1/Phase-2-Defer-Comment auf Modul-Ebene
- [x] Task 2 — `main.rs`-Bootstrap-Sequenz aktualisieren (AC-B, AC-C)
  - [x] 2.1 `load_default()`-Aufruf vor `tauri::Builder::default()` entfernen
  - [x] 2.2 Step-2b-Block nach Config-Load-Block: `let i18n_table = klarvo_windows_shell::i18n::load(&config.ui_language);`
  - [x] 2.3 Bootstrap-Step-Block-Comment um Step 2b erweitern
- [x] Task 3 — Tests (AC-D)
  - [x] 3.1 `load_en_returns_en_table` — kein TODO-Marker, EN-String
  - [x] 3.2 `load_de_returns_de_table` — Wert ≠ EN-String
  - [x] 3.3 `load_unknown_locale_falls_back_to_en` — identisch zu `load("en")`
  - [x] 3.4 `both_locale_files_valid_json_even_when_en_active` — eager-validation-Regression
- [x] Task 4 — Build + Tests verifizieren (AC-D, AC-G)
  - [x] 4.1 `cargo test -p klarvo-windows-shell --lib` grün: 13/13 pass (9 config + 4 i18n), 1 ignored

## Dev Agent Record

### Implementation Plan

1. `i18n.rs`: Hard-Replace `load_default()` → `load(ui_language: &str)`; Eager-Validation beider Files; Match-Arm `"de"` / `_`; Phase-1/2-Defer-Comment auf Modul-Level.
2. `main.rs`: Zeile vor `tauri::Builder::default()` entfernen (`let i18n_table = ...load_default()`); Step-2b-Block nach Config-Load-Block einsetzen; Bootstrap-Comment-Header um Step 2b ergänzen.
3. 4 Unit-Tests: load-en/load-de/load-xx/eager-validation.

### Completion Notes

- AC-A ✅ — `load(ui_language)` mit Eager-Validation + `"de"`/`_`-Match-Arm; `load_default()` gelöscht.
- AC-B ✅ — `load_default()`-Call vor Builder entfernt; Step-2b-Block nach Config-Block eingefügt; `app.manage(i18n_table)` (Step 11) unverändert.
- AC-C ✅ — Bootstrap-Step-Block-Comment um `Step 2b: i18n::load(ui_language)` ergänzt.
- AC-D ✅ — 13/13 Tests grün: `load_en_returns_en_table`, `load_de_returns_de_table`, `load_unknown_locale_falls_back_to_en`, `both_locale_files_valid_json_even_when_en_active`.
- AC-E ✅ — Phase-1/Phase-2-Defer-Comment auf Modul-Ebene in `i18n.rs`.
- AC-F ✅ — Frontend-Locales (`shells/windows/src/locales/`) unberührt.
- AC-G (bootstrap) ✅ — `cargo build`-Äquivalent: Test-Compilation erfolgreich (inkl. Linking).

## File List

- `shells/windows/src-tauri/src/i18n.rs` — `load_default()` → `load(ui_language)`; Eager-Validation + Match-Arm; Phase-Defer-Comment; 4 Unit-Tests.
- `shells/windows/src-tauri/src/main.rs` — `load_default()`-Aufruf entfernt; Step-2b-Block nach Config-Load; Bootstrap-Comment erweitert.
- `output/planning-artifacts/epics/epic-4/story-4.2-locale-aware-i18n-loading.md` — Status: review, Tasks/Subtasks + Dev Agent Record hinzugefügt.

## Change Log

- 2026-04-25: Story 4.2 implementiert — `i18n::load(ui_language)` Hard-Replace, Bootstrap Step-2b, 4 Unit-Tests, 13/13 grün.
