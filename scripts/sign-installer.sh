...
KLARVO_WIN="/mnt/d/Apps/klarvo"
KEY_FILE="$HOME/.tauri/voxlit.key"
DECODED_KEY="/tmp/klarvo-signing.key"

# Get version
if [ -n "$1" ]; then
    VERSION="$1"
else
    VERSION=$(python3 -c "import json; print(json.load(open('$KLARVO_WIN/package.json'))['version'])" 2>/dev/null \
        || jq -r .version "$KLARVO_WIN/package.json" 2>/dev/null)
fi
...
EXE="$KLARVO_WIN/src-tauri/target/release/bundle/nsis/Klarvo_${VERSION}_x64-setup.exe"
