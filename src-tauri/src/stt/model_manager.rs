...
mod tests {
    ...
    fn test_model_path_construction() {
        let dir = PathBuf::from("/tmp/klarvo-test-appdata");
        let path = model_path(&dir, "ggml-base.bin");
        assert_eq!(
            path,
            PathBuf::from("/tmp/klarvo-test-appdata/models/ggml-base.bin")
        );
    }
    ...
    fn test_get_model_status_not_downloaded() {
        ...
        let dir = PathBuf::from("/tmp/klarvo-test-no-models-12345");
    ...
    fn test_list_models_with_status_count() {
        let dir = PathBuf::from("/tmp/klarvo-no-models");
    ...
    fn test_delete_model_nonexistent_is_ok() {
        let dir = PathBuf::from("/tmp/klarvo-no-models-12345");
