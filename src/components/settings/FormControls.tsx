 ...
 * Renders dropdown via React portal to avoid overflow-hidden clipping.
 */
export function KSelect({ value, onChange, options, disabled }: KSelectProps) {
  ...
  return (
    <div className="relative w-full">
      ...
      {/* Portal-rendered dropdown — not clipped by panel overflow-hidden.
 ...
 */
export function KSlider({ value, onChange, min, max, step, disabled }: KSliderProps) {
  ...
  return (
    <input
      ...
      className={["k-slider w-full", disabled ? "opacity-40 cursor-not-allowed" : ""].join(" ")}
      style={{
 ...
 */
export function KSegmented({ value, onChange, options }: KSegmentedProps) {
  return (
    <div
      className="flex gap-0.5 bg-klarvo-bg rounded-klarvo-sm p-0.5 border border-klarvo-border"
    >
      {options.map((opt) => (
        <button
          key={opt.value}
