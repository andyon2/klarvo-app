...
export function DictionaryTag({ term, onRemove }: { term: string; onRemove: (t: string) => void }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 bg-klarvo-surface text-klarvo-muted pl-2.5 pr-1.5 py-1 rounded-full text-xs border border-klarvo-border/60">
      {term}
      <button
        onClick={() => onRemove(term)}
        className={[
          "text-klarvo-dim hover:text-klarvo-danger rounded-full transition-colors",
...
export function FillerStatsChart({ entries }: { entries: FillerEntry[] }) {
  if (entries.length === 0) {
    return <p className="text-xs text-klarvo-dim italic">No filler words tracked yet.</p>;
  }
    ...
    <div className="flex flex-col gap-1.5">
      {entries.slice(0, 10).map(({ word, count }) => (
        <div key={word} className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-klarvo-dim w-6 text-right shrink-0">{count}</span>
        </div>
      ))}
    </div>
  );
...
export function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-klarvo-surface border border-klarvo-border/60 rounded-xl p-3">
      <p className="text-[11px] text-klarvo-dim uppercase tracking-wide">{label}</p>
      <p className="text-lg font-semibold text-klarvo-text mt-0.5">
        {value}
        {sub && <span className="text-[11px] text-klarvo-dim font-normal ml-1">{sub}</span>}
      </p>
    </div>
  );
}
