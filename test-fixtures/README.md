...
| `guard-chain-vectors.json` | `pipeline.rs` (`spec_guard_*`, `spec_stt_prompt_join_is_explicit`, `spec_stt_hint_override_selection_falls_through_to_auto`, `spec_jni_guard_wrapper_agrees_with_the_desktop_chain`, plus the two that drive `process_audio` itself from a vector: `spec_process_audio_delivers_a_ghosted_transcript_minus_the_ghost` and `spec_process_audio_drops_a_punctuation_only_residue`) | `GuardChainBridgeTest` (the guard VERDICTS are n/a on Android by construction — ADR-0017 makes them shared Rust core, reached over `GroqSttBridge`; the Kotlin half asserts the DELEGATION, the vectors' own n/a DECLARATION — the count assertion is what turns "no Kotlin reader" into "deliberately none" — and drives the one vector that is a real twin, `STT-HINT-SELECT-*`) |
